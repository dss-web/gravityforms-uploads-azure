 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\Arrays;
 ...
 */
class ArrayIndentationSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
    ...
    public function register()
    {
         ...
         */
        $this->ignore_tokens = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$heredocTokens;
        unset($this->ignore_tokens[\T_START_HEREDOC], $this->ignore_tokens[\Dekode\GravityForms\Vendor\T_START_NOWDOC]);
        $this->ignore_tokens[\T_INLINE_HTML] = \T_INLINE_HTML;
        return array(\T_ARRAY, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        if (!isset($this->tab_width)) {
            $this->tab_width = \Dekode\GravityForms\Vendor\WordPressCS\WordPress\PHPCSHelper::get_tab_width($this->phpcsFile);
        }
        if (\Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code'] && $this->is_short_list($stackPtr)) {
        ...
        foreach ($array_items as $item) {
            ...
            $first_content = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_DOC_COMMENT_WHITESPACE), $item['start'], $end_of_this_item, \true);
            ...
            if (\false !== $first_content && \T_COMMENT === $this->tokens[$first_content]['code'] && $this->tokens[$first_content]['line'] === $this->tokens[$end_of_previous_item]['line']) {
                $first_content = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_DOC_COMMENT_WHITESPACE, \T_COMMENT), $first_content + 1, $end_of_this_item, \true);
            }
            ...
            $first_content_on_line2 = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_DOC_COMMENT_WHITESPACE), $ptr, $end_of_this_item, \true);
            if (\false === $first_content_on_line2) {
                 ...
                 */
                if ($this->tokens[$item['end']]['line'] !== $this->tokens[$end_of_this_item]['line'] && \Dekode\GravityForms\Vendor\T_COMMA === $this->tokens[$end_of_this_item]['code'] && $this->tokens[$end_of_this_item]['column'] - 1 !== $expected_spaces) {
            ...
            if ($found_spaces_on_line2 !== $expected_spaces_on_line2) {
                ...
                if (\true === $fix) {
                    ...
                    for ($i = $first_content_on_line2 + 1; $i <= $item['end']; $i++) {
                        ...
                        $first_content_on_line = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_DOC_COMMENT_WHITESPACE), $i, $end_of_this_item, \true);
    ...
    protected function ignore_token($ptr)
    {
         ...
         */
        if (\T_CONSTANT_ENCAPSED_STRING === $token_code || \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING === $token_code) {
    ...
    protected function get_indentation_size($ptr)
    {
        ...
        $whitespace = '';
        if (\T_WHITESPACE === $this->tokens[$ptr]['code'] || \Dekode\GravityForms\Vendor\T_DOC_COMMENT_WHITESPACE === $this->tokens[$ptr]['code']) {
