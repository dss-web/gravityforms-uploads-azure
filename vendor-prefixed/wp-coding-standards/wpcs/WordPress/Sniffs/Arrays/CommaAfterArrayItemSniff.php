 ...
 */
class CommaAfterArrayItemSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        foreach ($array_items as $item_index => $item) {
            ...
            $is_comma = \false;
            if (isset($this->tokens[$maybe_comma]) && \Dekode\GravityForms\Vendor\T_COMMA === $this->tokens[$maybe_comma]['code']) {
                $is_comma = \true;
            }
            /*
            ...
            $last_content = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $item['end'], $item['start'], \true);
             ...
             */
            if ($last_content !== $item['end'] && (\true === $single_line || !isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$heredocTokens[$this->tokens[$last_content]['code']]))) {
                $newlines = 0;
                $spaces = 0;
                for ($i = $item['end']; $i > $last_content; $i--) {
                    ...
                    } elseif (\T_COMMENT === $this->tokens[$i]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$i]['code']])) {
                        break;
                    }
                }
                $space_phrases = array();
            ...
            if (\T_WHITESPACE === $next_token['code']) {
                ...
                if (\false === $next_non_whitespace || \false === $single_line && $this->tokens[$next_non_whitespace]['line'] === $this->tokens[$maybe_comma]['line'] && (\T_COMMENT === $this->tokens[$next_non_whitespace]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$next_non_whitespace]['code']]))) {
                    continue;
                }
