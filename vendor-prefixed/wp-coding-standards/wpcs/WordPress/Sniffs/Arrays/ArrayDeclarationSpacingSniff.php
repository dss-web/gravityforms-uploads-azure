 ...
 */
class ArrayDeclarationSpacingSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
     ...
     * @var array
     */
    private $targets = array(\T_ARRAY => \T_ARRAY, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY => \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY);
    /**
                ...
                if (\true === $array_has_keys) {
                    ...
                    if (\true === $fix) {
                        ...
                        foreach ($array_items as $item) {
                             ...
                             */
                            $first_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $item['start'], $item['end'] + 1, \true);
    ...
    protected function process_multi_line_array($stackPtr, $opener, $closer)
    {
        ...
        foreach ($array_items as $item) {
            ...
            if ($this->tokens[$first_content]['line'] === $this->tokens[$end_of_last_item]['line'] && (\T_COMMENT === $this->tokens[$first_content]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$first_content]['code']]))) {
                ...
                if (\substr(\ltrim($this->tokens[$end_of_comment]['content']), 0, 2) === '/*') {
                    while ((\T_COMMENT === $this->tokens[$end_of_comment]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$end_of_comment]['code']])) && \substr(\rtrim($this->tokens[$end_of_comment]['content']), -2) !== '*/' && $end_of_comment + 1 < $end_of_this_item) {
                        $end_of_comment++;
                    }
                ...
                $next = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_DOC_COMMENT_WHITESPACE), $end_of_comment + 1, $end_of_this_item, \true);
                if (\false === $next) {
