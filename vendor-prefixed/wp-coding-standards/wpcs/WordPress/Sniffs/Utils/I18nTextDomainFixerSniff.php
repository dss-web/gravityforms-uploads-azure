 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\Utils;
 ...
 * @since   1.2.0
 */
class I18nTextDomainFixerSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\AbstractFunctionParameterSniff
{
    /**
    ...
    public function register()
    {
        ...
        $targets[] = \Dekode\GravityForms\Vendor\T_DOC_COMMENT_OPEN_TAG;
        $targets[] = \T_COMMENT;
        return $targets;
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\Dekode\GravityForms\Vendor\T_DOC_COMMENT_OPEN_TAG === $this->tokens[$stackPtr]['code'] || \T_COMMENT === $this->tokens[$stackPtr]['code']) {
    ...
    public function process_no_parameters($stackPtr, $group_name, $matched_content)
    {
        ...
        $opener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS !== $this->tokens[$opener]['code'] || isset($this->tokens[$opener]['parenthesis_closer']) === \false) {
    ...
    {
        if (\true === $this->header_found && !\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS')) {
            return;
        }
        ...
        if ('CSS' === $this->phpcsFile->tokenizerType) {
            if ('style.css' !== $file_name && !\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS')) {
        ...
        if (\T_COMMENT === $this->tokens[$stackPtr]['code']) {
            ...
            $current = $stackPtr;
            while (($current = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_DOC_COMMENT_STRING, $current + 1, $closer)) !== \false) {
                $comment_details = $this->examine_comment_line($current, $regex, $headers, $comment_details);
        ...
        if (\true === $comment_details['required_header_found'] && $comment_details['headers_found'] >= 3) {
            ...
            if (\false !== $text_domain_ptr) {
                ...
                if (\true === $fix) {
                    ...
                    if (\Dekode\GravityForms\Vendor\T_DOC_COMMENT_OPEN_TAG === $this->tokens[$stackPtr]['code']) {
