 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\Files;
 ...
 */
class FileNameSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
    ...
    {
        if (\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS')) {
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\defined('Dekode\\GravityForms\\Vendor\\T_PHPCS_DISABLE') && \defined('Dekode\\GravityForms\\Vendor\\T_PHPCS_ENABLE')) {
            $i = -1;
            while ($i = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_PHPCS_DISABLE, $i + 1)) {
                if (empty($this->tokens[$i]['sniffCodes']) || isset($this->tokens[$i]['sniffCodes']['WordPress']) || isset($this->tokens[$i]['sniffCodes']['WordPress.Files']) || isset($this->tokens[$i]['sniffCodes']['WordPress.Files.FileName'])) {
                    do {
                        $i = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_PHPCS_ENABLE, $i + 1);
        ...
        if (\false !== \strpos($file, \DIRECTORY_SEPARATOR . 'wp-includes' . \DIRECTORY_SEPARATOR)) {
            $subpackage_tag = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_DOC_COMMENT_TAG, $stackPtr, null, \false, '@subpackage');
            if (\false !== $subpackage_tag) {
                $subpackage = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_DOC_COMMENT_STRING, $subpackage_tag);
                if (\false !== $subpackage) {
                    $fileName_end = \substr($fileName, -13);
                    $has_class = $this->phpcsFile->findNext(\T_CLASS, $stackPtr);
                    if ('Template' === \trim($this->tokens[$subpackage]['content']) && $this->tokens[$subpackage_tag]['line'] === $this->tokens[$subpackage]['line'] && (!\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS') && '-template.php' !== $fileName_end || \defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS') && '-template.inc' !== $fileName_end) && \false === $has_class) {
