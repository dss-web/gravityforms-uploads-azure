 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\WhiteSpace;
 ...
 */
class ControlStructureSpacingSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
     ...
     * @var array
     */
    private $ignore_extra_space_after_open_paren = array(\T_FUNCTION => \true, \Dekode\GravityForms\Vendor\T_CLOSURE => \true, \T_DO => \true, \T_ELSE => \true, \T_TRY => \true);
    /**
    ...
    {
        return array(\T_IF, \T_WHILE, \T_FOREACH, \T_FOR, \T_SWITCH, \T_DO, \T_ELSE, \T_ELSEIF, \T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE, \T_USE, \T_TRY, \T_CATCH);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (isset($this->tokens[$stackPtr + 1]) && \T_WHITESPACE !== $this->tokens[$stackPtr + 1]['code'] && !(\T_ELSE === $this->tokens[$stackPtr]['code'] && \Dekode\GravityForms\Vendor\T_COLON === $this->tokens[$stackPtr + 1]['code']) && !(\Dekode\GravityForms\Vendor\T_CLOSURE === $this->tokens[$stackPtr]['code'] && 0 >= $this->spaces_before_closure_open_paren)) {
        ...
        if (!isset($this->tokens[$stackPtr]['scope_closer'])) {
            if (\T_USE === $this->tokens[$stackPtr]['code'] && 'closure' === $this->get_use_type($stackPtr)) {
                $scopeOpener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET, $stackPtr + 1);
        ...
        // Alternative syntax.
        if (isset($scopeOpener) && \Dekode\GravityForms\Vendor\T_COLON === $this->tokens[$scopeOpener]['code']) {
        ...
        $parenthesisOpener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if (\T_FUNCTION === $this->tokens[$stackPtr]['code']) {
            ...
            } elseif (\Dekode\GravityForms\Vendor\T_BITWISE_AND === $this->tokens[$parenthesisOpener]['code']) {
                // This function returns by reference (function &function_name() {}).
                $parenthesisOpener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $parenthesisOpener + 1, null, \true);
                $function_name_ptr = $parenthesisOpener;
            }
        ...
        } elseif (\Dekode\GravityForms\Vendor\T_CLOSURE === $this->tokens[$stackPtr]['code']) {
            // Check if there is a use () statement.
            if (isset($this->tokens[$parenthesisOpener]['parenthesis_closer'])) {
                $usePtr = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $this->tokens[$parenthesisOpener]['parenthesis_closer'] + 1, null, \true, null, \true);
        ...
        if (\Dekode\GravityForms\Vendor\T_COLON !== $this->tokens[$parenthesisOpener]['code'] && \T_FUNCTION !== $this->tokens[$stackPtr]['code']) {
            if (\Dekode\GravityForms\Vendor\T_CLOSURE === $this->tokens[$stackPtr]['code'] && 0 === $this->spaces_before_closure_open_paren) {
            ...
            } elseif ((\Dekode\GravityForms\Vendor\T_CLOSURE !== $this->tokens[$stackPtr]['code'] || 1 === $this->spaces_before_closure_open_paren) && $stackPtr + 1 === $parenthesisOpener) {
                // Checking this: if[*](...) {}.
        ...
        if (\Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
        ...
        if (isset($this->tokens[$parenthesisOpener]['parenthesis_closer'])) {
            ...
            if (\Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
                ...
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser - 1]['code']) {
                    ...
                    $prevNonEmpty = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $parenthesisCloser - 1, null, \true);
                ...
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser + 1]['code'] && !(\Dekode\GravityForms\Vendor\T_COLON === $this->tokens[$parenthesisCloser + 1]['code'] && (isset($this->tokens[$parenthesisCloser]['parenthesis_owner']) === \false || \in_array($this->tokens[$this->tokens[$parenthesisCloser]['parenthesis_owner']]['code'], array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE), \true))) && (isset($scopeOpener) && \Dekode\GravityForms\Vendor\T_COLON !== $this->tokens[$scopeOpener]['code'])) {
            ...
            if (\T_FUNCTION !== $this->tokens[$stackPtr]['code'] && \Dekode\GravityForms\Vendor\T_CLOSURE !== $this->tokens[$stackPtr]['code'] && isset($this->tokens[$parenthesisOpener]['parenthesis_owner']) && (isset($scopeOpener) && $this->tokens[$parenthesisCloser]['line'] !== $this->tokens[$scopeOpener]['line'])) {
        ...
        if (\false !== $this->blank_line_check && isset($scopeOpener)) {
            ...
            $ignore = array(\T_FUNCTION => \true, \Dekode\GravityForms\Vendor\T_CLOSURE => \true, \Dekode\GravityForms\Vendor\T_DOC_COMMENT_OPEN_TAG => \true, \T_CLOSE_TAG => \true, \T_COMMENT => \true);
            $ignore += \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens;
            ...
            if ($firstContent !== $scopeCloser) {
                ...
                $lastNonEmptyContent = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $scopeCloser - 1, null, \true);
                    ...
                    for ($i = $scopeCloser - 1; $i > $lastContent; $i--) {
                        if ($this->tokens[$i]['line'] < $this->tokens[$scopeCloser]['line'] && \T_OPEN_TAG !== $this->tokens[$firstContent]['code']) {
                            ...
                            if (\true === $fix) {
                                 ...
                                 * conflict.
                                 */
                                if (\T_COMMENT !== $this->tokens[$lastContent]['code'] && !isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$lastContent]['code']])) {
        ...
        if (\T_COMMENT === $this->tokens[$trailingContent]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$trailingContent]['code']])) {
            // Special exception for code where the comment about
            // an ELSE or ELSEIF is written between the control structures.
            $nextCode = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $scopeCloser + 1, null, \true);
        ...
        if (isset($this->tokens[$trailingContent]['scope_condition']) && \Dekode\GravityForms\Vendor\T_CLOSE_CURLY_BRACKET === $this->tokens[$trailingContent]['code']) {
            // Another control structure's closing brace.
            $owner = $this->tokens[$trailingContent]['scope_condition'];
            if (\in_array($this->tokens[$owner]['code'], array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE, \T_CLASS, \Dekode\GravityForms\Vendor\T_ANON_CLASS, \T_INTERFACE, \T_TRAIT), \true)) {
            ...
            if ($this->tokens[$scopeCloser]['line'] + 1 !== $this->tokens[$trailingContent]['line']) {
                ...
                if (\true === $fix) {
                    ...
                    if (\T_COMMENT !== $this->tokens[$scopeCloser]['code'] && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$scopeCloser]['code']]) === \false) {
