...
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\AbstractVariableSniff as PHPCS_AbstractVariableSniff;
 ...
 */
class ValidVariableNameSniff extends \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\AbstractVariableSniff
{
    /**
     ...
     * @return void
     */
    protected function processVariable(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcs_file, $stack_ptr)
    {
        ...
        $obj_operator = $phpcs_file->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stack_ptr + 1, null, \true);
        if (\T_OBJECT_OPERATOR === $tokens[$obj_operator]['code']) {
            // Check to see if we are using a variable from an object.
            $var = $phpcs_file->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $obj_operator + 1, null, \true);
            if (\T_STRING === $tokens[$var]['code']) {
                $bracket = $phpcs_file->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $var + 1, null, \true);
                if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS !== $tokens[$bracket]['code']) {
                    ...
                    if (!isset($this->whitelisted_mixed_case_member_var_names[$obj_var_name]) && self::isSnakeCase($obj_var_name) === \false) {
                        ...
                        $data = array($original_var_name, \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff::get_snake_case_name_suggestion($original_var_name));
        ...
        $in_class = \false;
        $obj_operator = $phpcs_file->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stack_ptr - 1, null, \true);
     ...
     * @return void
     */
    protected function processMemberVar(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcs_file, $stack_ptr)
    {
        $tokens = $phpcs_file->getTokens();
        $var_name = \ltrim($tokens[$stack_ptr]['content'], '$');
        ...
        if (!isset($this->whitelisted_mixed_case_member_var_names[$var_name]) && \false === self::isSnakeCase($var_name)) {
            ...
            $data = array($var_name, \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff::get_snake_case_name_suggestion($var_name));
     ...
     * @return void
     */
    protected function processVariableInString(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcs_file, $stack_ptr)
    {
        $tokens = $phpcs_file->getTokens();
    ...
    protected function mergeWhiteList()
    {
        if ($this->customPropertiesWhitelist !== $this->addedCustomProperties['properties']) {
            ...
            $customProperties = \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff::merge_custom_array($this->customPropertiesWhitelist, array(), \false);
            $this->whitelisted_mixed_case_member_var_names = \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff::merge_custom_array($customProperties, $this->whitelisted_mixed_case_member_var_names);
