 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\NamingConventions;

use Dekode\GravityForms\Vendor\WordPressCS\WordPress\AbstractFunctionParameterSniff;
 ...
 */
class PrefixAllGlobalsSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\AbstractFunctionParameterSniff
{
    /**
    ...
    public function register()
    {
        ...
        $targets = array(
            ...
            \Dekode\GravityForms\Vendor\T_DOLLAR => \Dekode\GravityForms\Vendor\T_DOLLAR,
            // Variable variables.
            \T_LIST => \T_LIST,
            \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY => \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY,
        );
        $targets += \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens;
    ...
    public function process_token($stackPtr)
    {
        ...
        $cl_prefixes = \trim(\Dekode\GravityForms\Vendor\WordPressCS\WordPress\PHPCSHelper::get_config_data('prefixes'));
        ...
        // Ignore test classes.
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[$this->tokens[$stackPtr]['code']]) && \true === $this->is_test_class($stackPtr)) {
        ...
        if (\Dekode\GravityForms\Vendor\T_ANON_CLASS === $this->tokens[$stackPtr]['code']) {
        ...
        if (\T_STRING === $this->tokens[$stackPtr]['code']) {
        ...
        } elseif (\Dekode\GravityForms\Vendor\T_DOLLAR === $this->tokens[$stackPtr]['code']) {
        ...
        } elseif (\T_LIST === $this->tokens[$stackPtr]['code'] || \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code']) {
            ...
            switch ($this->tokens[$stackPtr]['type']) {
                case 'T_FUNCTION':
                    ...
                    if ($this->phpcsFile->hasCondition($stackPtr, \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens) === \true) {
                        return;
                    }
                ...
                case 'T_CONST':
                    ...
                    $constant_name_ptr = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
    ...
    {
        static $indicators = array(\Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET => \true, \T_VARIABLE => \true);
        ...
        if (\Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET === $this->tokens[$next_non_empty]['code'] && isset($this->tokens[$next_non_empty]['bracket_closer'])) {
            // Skip over the variable part.
            $next_non_empty = $this->tokens[$next_non_empty]['bracket_closer'];
        }
        $maybe_assignment = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next_non_empty + 1, null, \true, null, \true);
        while (\false !== $maybe_assignment && \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET === $this->tokens[$maybe_assignment]['code'] && isset($this->tokens[$maybe_assignment]['bracket_closer'])) {
            $maybe_assignment = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $this->tokens[$maybe_assignment]['bracket_closer'] + 1, null, \true, null, \true);
        }
        if (\false === $maybe_assignment) {
            return;
        }
        if (!isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$assignmentTokens[$this->tokens[$maybe_assignment]['code']])) {
            // Not an assignment.
            return;
        }
         ...
         */
        if ($this->phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE)) === \true) {
            $condition = $this->phpcsFile->getCondition($stackPtr, \T_FUNCTION);
            if (\false === $condition) {
                $condition = $this->phpcsFile->getCondition($stackPtr, \Dekode\GravityForms\Vendor\T_CLOSURE);
            }
    ...
    protected function process_variable_assignment($stackPtr, $in_list = \false)
    {
        ...
        if ('GLOBALS' === $variable_name) {
            $array_open = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
            if (\false === $array_open || \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET !== $this->tokens[$array_open]['code']) {
                // Live coding or something very silly.
                return;
            }
            $array_key = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $array_open + 1, null, \true, null, \true);
            if (\false === $array_key) {
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$stringTokens[$this->tokens[$array_key]['code']]) && $this->variable_prefixed_or_whitelisted($stackPtr, $variable_name) === \true) {
                return;
            }
            if (\Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING === $this->tokens[$array_key]['code']) {
            ...
            } elseif (!isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$stringTokens[$this->tokens[$array_key]['code']])) {
            ...
            if (\false === $in_list && isset($this->tokens[$stackPtr]['nested_parenthesis'])) {
                foreach ($this->tokens[$stackPtr]['nested_parenthesis'] as $opener => $closer) {
                    if (isset($this->tokens[$opener]['parenthesis_owner']) && (\T_FUNCTION === $this->tokens[$this->tokens[$opener]['parenthesis_owner']]['code'] || \Dekode\GravityForms\Vendor\T_CLOSURE === $this->tokens[$this->tokens[$opener]['parenthesis_owner']]['code'])) {
                        return;
                    }
                }
                unset($opener, $closer);
            ...
            if ($this->phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE)) === \true) {
                ...
                $end_of_statement = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_SEMICOLON, $has_global + 1);
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if ($this->is_prefixed($parameters[1]['start'], $raw_content) === \true) {
            ...
            $first_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $parameters[1]['start'], $parameters[1]['end'] + 1, \true);
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$stringTokens[$this->tokens[$first_non_empty]['code']]) && $this->is_prefixed($parameters[1]['start'], $first_non_empty_content) === \true) {
                return;
            }
            if (\Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING === $this->tokens[$first_non_empty]['code']) {
            ...
            } elseif (!isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$stringTokens[$this->tokens[$first_non_empty]['code']])) {
