 ...
 */
class DirectDatabaseQuerySniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $endOfStatement = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_SEMICOLON, $stackPtr + 1, null, \false, null, \true);
        $endOfLineComment = '';
        ...
        for ($_pos = $stackPtr + 1; $_pos < $endOfStatement; $_pos++) {
            $_pos = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$textStringTokens, $_pos, $endOfStatement, \false, null, \true);
            if (\false === $_pos) {
                break;
            }
        ...
        if (!$whitelisted_cache && !empty($this->tokens[$stackPtr]['conditions'])) {
            ...
            if (\false === $scope_function) {
                $scope_function = $this->phpcsFile->getCondition($stackPtr, \Dekode\GravityForms\Vendor\T_CLOSURE);
            }
            if (\false !== $scope_function) {
