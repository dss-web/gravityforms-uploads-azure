 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\DB;
 ...
 * @since   0.14.0
 */
class PreparedSQLPlaceholdersSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        for ($i = $query['start']; $i <= $query['end']; $i++) {
            ...
            if (!isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$textStringTokens[$this->tokens[$i]['code']])) {
                ...
                if (\T_STRING === $this->tokens[$i]['code']) {
                    if ('sprintf' === \strtolower($this->tokens[$i]['content'])) {
                        ...
                        $prev = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$textStringTokens, $i - 1, $query['start']);
                        ...
                        if (\preg_match('`\\s+IN\\s*\\(\\s*(' . $regex_quote . ')?$`i', $prev_content, $match) > 0) {
                            ...
                            if ($this->analyse_implode($i) === \true) {
                                ...
                                $next = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
                                if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS === $this->tokens[$next]['code'] && isset($this->tokens[$next]['parenthesis_closer'])) {
                                    $skip_from = $i + 1;
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$stringTokens[$this->tokens[$i]['code']])) {
                $content = $this->strip_quotes($content);
                $regex_quote = $this->get_regex_quote_snippet($content, $this->tokens[$i]['content']);
            }
            if (\Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING === $this->tokens[$i]['code'] || \Dekode\GravityForms\Vendor\T_HEREDOC === $this->tokens[$i]['code']) {
        ...
        if (isset($parameters[2]) && 2 === $total_parameters) {
            $next = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $parameters[2]['start'], $parameters[2]['end'] + 1, \true);
            if (\false !== $next && (\T_ARRAY === $this->tokens[$next]['code'] || \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY === $this->tokens[$next]['code'])) {
    ...
    protected function analyse_sprintf($sprintf_params)
    {
        ...
        foreach ($sprintf_params as $sprintf_param) {
            ...
            $implode = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $sprintf_param['start'], $sprintf_param['end'], \true);
    ...
    protected function analyse_implode($implode_token)
    {
        ...
        $array_fill = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $implode_params[2]['start'], $implode_params[2]['end'], \true);
