 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniffs\Security;
 ...
 */
class EscapeOutputSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $open_paren = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if (!isset($end_of_statement)) {
            $end_of_statement = $this->phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_SEMICOLON, \T_CLOSE_TAG), $stackPtr);
            $last_token = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end_of_statement - 1, null, \true);
            ...
            if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS !== $this->tokens[$open_paren]['code'] || \Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS !== $this->tokens[$last_token]['code']) {
                $ternary = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_INLINE_THEN, $stackPtr, $end_of_statement);
        ...
        for ($i = $stackPtr; $i < $end_of_statement; $i++) {
            ...
            if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS === $this->tokens[$i]['code']) {
                ...
                if ($in_cast) {
                    ...
                    $ternary = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_INLINE_THEN, $i, $this->tokens[$i]['parenthesis_closer']);
                    if (\false !== $ternary) {
                        $next_paren = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS, $i + 1, $this->tokens[$i]['parenthesis_closer']);
            ...
            if (\Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY === $this->tokens[$i]['code'] || \Dekode\GravityForms\Vendor\T_CLOSE_SHORT_ARRAY === $this->tokens[$i]['code']) {
                continue;
            }
            if (\in_array($this->tokens[$i]['code'], array(\T_DOUBLE_ARROW, \Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS), \true)) {
                continue;
            }
            ...
            if (\Dekode\GravityForms\Vendor\T_STRING_CONCAT === $this->tokens[$i]['code']) {
                $watch = \true;
                continue;
            }
            // Wake up after a ternary else (:).
            if (\false !== $ternary && \Dekode\GravityForms\Vendor\T_INLINE_ELSE === $this->tokens[$i]['code']) {
                $watch = \true;
                continue;
            }
            // Wake up for commas.
            if (\Dekode\GravityForms\Vendor\T_COMMA === $this->tokens[$i]['code']) {
                $in_cast = \false;
                $watch = \true;
                continue;
            }
            ...
            if (\T_STRING === $this->tokens[$i]['code']) {
                ...
                $function_opener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS, $i + 1, null, \false, null, \true);
                ...
                if (\false !== $function_opener) {
                    if (isset($this->arrayWalkingFunctions[$functionName])) {
                        ...
                        if (!empty($callback)) {
                             ...
                             */
                            $mapped_function = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $callback['start'], $callback['end'] + 1, \true);
