 ...
 */
namespace Dekode\GravityForms\Vendor\WordPressCS\WordPress;

use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff as PHPCS_Sniff;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens;
use Dekode\GravityForms\Vendor\WordPressCS\WordPress\PHPCSHelper;
/**
 * Represents a PHP_CodeSniffer sniff for sniffing WordPress coding standards.
 *
 ...
 */
abstract class Sniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    protected $test_class_whitelist = array(
        ...
        'Dekode\\GravityForms\\Vendor\\PHPUnit\\Framework\\TestCase' => \true,
     ...
     */
    protected function init(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile)
    {
    ...
    {
        $cl_supported_version = \trim(\Dekode\GravityForms\Vendor\WordPressCS\WordPress\PHPCSHelper::get_config_data('minimum_supported_wp_version'));
    ...
    protected function has_whitelist_comment($comment, $stackPtr)
    {
        ...
        if (\true === \Dekode\GravityForms\Vendor\WordPressCS\WordPress\PHPCSHelper::ignore_annotations($this->phpcsFile)) {
            return \false;
        }
        ...
        $end_of_statement = $this->phpcsFile->findNext(array(\T_CLOSE_TAG, \Dekode\GravityForms\Vendor\T_SEMICOLON), $stackPtr);
        if (\false !== $end_of_statement) {
            ...
            if (\Dekode\GravityForms\Vendor\T_SEMICOLON === $this->tokens[$end_of_statement]['code']) {
            ...
            if ((\T_COMMENT === $this->tokens[$lastPtr]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) && \Dekode\GravityForms\Vendor\T_PHPCS_SET !== $this->tokens[$lastPtr]['code']) && $this->tokens[$lastPtr]['line'] === $this->tokens[$end_of_statement]['line'] && \preg_match($regex, $this->tokens[$lastPtr]['content']) === 1) {
                if (isset($thrown_notices[$filename][$lastPtr]) === \false && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) === \false) {
        ...
        if ((\T_COMMENT === $this->tokens[$lastPtr]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) && \Dekode\GravityForms\Vendor\T_PHPCS_SET !== $this->tokens[$lastPtr]['code']) && $this->tokens[$lastPtr]['line'] === $this->tokens[$stackPtr]['line'] && \preg_match($regex, $this->tokens[$lastPtr]['content']) === 1) {
            if (isset($thrown_notices[$filename][$lastPtr]) === \false && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) === \false) {
                $this->phpcsFile->addWarning($deprecation_notice, $lastPtr, $deprecation_code, array($this->tokens[$lastPtr]['content']));
                $thrown_notices[$filename][$lastPtr] = \true;
            }
    ...
    protected function is_token_in_test_method($stackPtr)
    {
        ...
        foreach ($conditions as $token => $condition) {
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[$condition])) {
    ...
    {
        if (isset($this->tokens[$stackPtr], \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[$this->tokens[$stackPtr]['code']]) === \false) {
            return \false;
        }
    ...
    {
        static $valid = array(\T_VARIABLE => \true, \Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET => \true);
        // Must be a variable, constant or closing square bracket (see below).
        if (!isset($valid[$this->tokens[$stackPtr]['code']])) {
            return \false;
        }
        $next_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        // No token found.
        if (\false === $next_non_empty) {
            return \false;
        }
        // If the next token is an assignment, that's all we need to know.
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$assignmentTokens[$this->tokens[$next_non_empty]['code']])) {
            return \true;
        }
        // Check if this is an array assignment, e.g., `$var['key'] = 'val';` .
        if (\Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET === $this->tokens[$next_non_empty]['code'] && isset($this->tokens[$next_non_empty]['bracket_closer'])) {
    ...
    protected function has_nonce_check($stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['conditions']) === \true) {
            ...
            foreach ($conditions as $tokenPtr => $condition) {
                if (\T_FUNCTION === $condition || \Dekode\GravityForms\Vendor\T_CLOSURE === $condition) {
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if (\T_FUNCTION === $tokens[$i]['code'] || \Dekode\GravityForms\Vendor\T_CLOSURE === $tokens[$i]['code'] || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[$tokens[$i]['code']])) {
    ...
    protected function is_in_isset_or_empty($stackPtr)
    {
        ...
        $previous_non_empty = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $open_parenthesis - 1, null, \true, null, \true);
    ...
    {
        $before = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
        if (\false === $before) {
            return \false;
        }
    ...
    {
        $prev = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
        if (\false === $prev) {
            return \false;
        }
        ...
        $before_prev = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $prev - 1, null, \true, null, \true);
    ...
    protected function is_in_function_call($stackPtr, $valid_functions, $global = \true, $allow_nested = \false)
    {
        ...
        foreach ($nested_parenthesis as $open => $close) {
            $prev_non_empty = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $open - 1, null, \true, null, \true);
    ...
    protected function is_sanitized($stackPtr, $require_unslash = \false)
    {
        ...
        $functionPtr = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $function_opener - 1, null, \true, null, \true);
        ...
        if (isset($this->arrayWalkingFunctions[$functionName])) {
            ...
            if (!empty($callback)) {
                 ...
                 */
                $first_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $callback['start'], $callback['end'] + 1, \true);
    ...
    protected function get_array_access_keys($stackPtr, $all = \true)
    {
        ...
        do {
            ...
            $open_bracket = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $current + 1, null, \true);
            // If it isn't a bracket, this isn't an array-access.
            if (\false === $open_bracket || \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET !== $this->tokens[$open_bracket]['code'] || !isset($this->tokens[$open_bracket]['bracket_closer'])) {
                break;
            }
    ...
    protected function is_validated($stackPtr, $array_keys = array(), $in_condition_only = \false)
    {
        if ($in_condition_only) {
            ...
            if (\false !== $function) {
                ...
                $closure = $this->phpcsFile->getCondition($stackPtr, \Dekode\GravityForms\Vendor\T_CLOSURE);
        ...
        for ($i = $scope_start + 1; $i < $scope_end; $i++) {
            ...
            switch ($targets[$this->tokens[$i]['code']]) {
                case 'construct':
                    $issetOpener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true, null, \true);
                    if (\false === $issetOpener || \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS !== $this->tokens[$issetOpener]['code']) {
                ...
                case 'function_call':
                    ...
                    $next_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true, null, \true);
                    if (\false === $next_non_empty || \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS !== $this->tokens[$next_non_empty]['code']) {
                        // Not a function call.
                        continue 2;
                    }
                    if ($this->is_class_object_call($i) === \true) {
                    ...
                    $param2_first_token = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $params[2]['start'], $params[2]['end'] + 1, \true);
                ...
                case 'coalesce':
                    $prev = $i;
                    do {
                        $prev = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $prev - 1, null, \true, null, \true);
                        // Skip over array keys, like $_GET['key']['subkey'].
                        if (\Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET === $this->tokens[$prev]['code']) {
    ...
    {
        $comparisonTokens = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$comparisonTokens;
        ...
        $previous_token = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (isset($comparisonTokens[$this->tokens[$previous_token]['code']])) {
            return \true;
        }
        // Maybe the comparison operator is after this.
        $next_token = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        // This might be an opening square bracket in the case of arrays ($var['a']).
        while (\false !== $next_token && \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET === $this->tokens[$next_token]['code']) {
            $next_token = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $this->tokens[$next_token]['bracket_closer'] + 1, null, \true);
        }
    ...
    protected function get_use_type($stackPtr)
    {
        ...
        if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS === $this->tokens[$next]['code']) {
            return 'closure';
        }
    ...
    public function does_function_call_have_parameters($stackPtr)
    {
        ...
        if (\false === \in_array($this->tokens[$stackPtr]['code'], array(\T_STRING, \T_ARRAY, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY), \true)) {
            return \false;
        }
        $next_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        ...
        $next_next_non_empty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next_non_empty + 1, $close_parenthesis + 1, \true);
    ...
    public function get_function_call_parameters($stackPtr)
    {
        ...
        if ('T_OPEN_SHORT_ARRAY' === $this->tokens[$stackPtr]['type']) {
            ...
            $opener = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        ...
        $cnt = 1;
        while ($next_comma = $this->phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_COMMA, $this->tokens[$closer]['code'], \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, \Dekode\GravityForms\Vendor\T_CLOSURE), $next_comma + 1, $closer + 1)) {
            ...
            if (\Dekode\GravityForms\Vendor\T_COMMA === $this->tokens[$next_comma]['code'] && isset($this->tokens[$next_comma]['nested_parenthesis']) && \count($this->tokens[$next_comma]['nested_parenthesis']) !== $nestedParenthesisCount) {
                continue;
            }
             ...
             */
            $has_next_param = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next_comma + 1, $closer, \true, null, \true);
    ...
    protected function find_list_open_close($stackPtr)
    {
        ...
        if (\Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code'] && $this->is_short_list($stackPtr) === \true) {
            $opener = $stackPtr;
            $closer = $this->tokens[$stackPtr]['bracket_closer'];
        }
        if (isset($opener, $closer)) {
    ...
    public function get_declared_namespace_name($stackPtr)
    {
        ...
        $nextToken = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if (\T_NS_SEPARATOR === $this->tokens[$nextToken]['code']) {
            // Not a namespace declaration, but use of, i.e. `namespace\someFunction();`.
            return \false;
        }
        if (\Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET === $this->tokens[$nextToken]['code']) {
        ...
        $validTokens = $acceptedTokens + \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
        $namespaceName = '';
    ...
    protected function is_wpdb_method_call($stackPtr, $target_methods)
    {
        ...
        if (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS !== $this->tokens[$opening_paren]['code'] || !isset($this->tokens[$opening_paren]['parenthesis_closer'])) {
            return \false;
        }
        ...
        if (\Dekode\GravityForms\Vendor\T_COMMA !== $this->tokens[$end]['code']) {
            ++$end;
        }
    ...
    public function is_use_of_global_constant($stackPtr)
    {
        ...
        $next = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\false !== $next && (\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS === $this->tokens[$next]['code'] || \T_DOUBLE_COLON === $this->tokens[$next]['code'])) {
        ...
        $firstOnLine = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
        if (\false !== $firstOnLine && \T_USE === $this->tokens[$firstOnLine]['code']) {
            $nextOnLine = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $firstOnLine + 1, null, \true);
            if (\false !== $nextOnLine) {
    ...
    {
        // Is this one of the tokens this function handles ?
        if (\Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY !== $this->tokens[$stackPtr]['code'] && \Dekode\GravityForms\Vendor\T_CLOSE_SHORT_ARRAY !== $this->tokens[$stackPtr]['code']) {
            return \false;
        }
        ...
        $nextNonEmpty = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $closer + 1, null, \true, null, \true);
        if (\false !== $nextNonEmpty && \Dekode\GravityForms\Vendor\T_EQUAL === $this->tokens[$nextNonEmpty]['code']) {
            return \true;
        }
        ...
        $parentOpen = $opener;
        do {
            $parentOpen = $this->phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, $parentOpen - 1, null, \false, null, \true);
            if (\false === $parentOpen) {
                return \false;
            }
    ...
    {
        if (\T_LIST !== $this->tokens[$stackPtr]['code'] && \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY !== $this->tokens[$stackPtr]['code']) {
            return array();
        }
        ...
        do {
            ++$current;
            $next_comma = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_COMMA, $current, $closer);
            if (\false === $next_comma) {
                $next_comma = $closer;
                $last = \true;
            }
     ...
     * @return bool
     */
    public static function is_function_deprecated(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if (\Dekode\GravityForms\Vendor\T_DOC_COMMENT_CLOSE_TAG !== $tokens[$comment_end]['code']) {
