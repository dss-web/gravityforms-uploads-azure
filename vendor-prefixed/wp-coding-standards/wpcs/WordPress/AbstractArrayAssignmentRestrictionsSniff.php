 ...
 */
abstract class AbstractArrayAssignmentRestrictionsSniff extends \Dekode\GravityForms\Vendor\WordPressCS\WordPress\Sniff
{
    /**
     * Exclude groups.
     *
    ...
    public function register()
    {
        ...
        return array(\T_DOUBLE_ARROW, \Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET, \T_CONSTANT_ENCAPSED_STRING, \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET === $token['code']) {
            $equal = $this->phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 1, null, \true);
            if (\Dekode\GravityForms\Vendor\T_EQUAL !== $this->tokens[$equal]['code']) {
                return;
         ...
         * $foo['bar'] = $taz;
         */
        if (\in_array($token['code'], array(\Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET, \T_DOUBLE_ARROW), \true)) {
            $operator = $stackPtr;
            // T_DOUBLE_ARROW.
            if (\Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET === $token['code']) {
                $operator = $this->phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_EQUAL, $stackPtr + 1);
            }
            $keyIdx = $this->phpcsFile->findPrevious(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET), $operator - 1, null, \true);
            if (!\is_numeric($this->tokens[$keyIdx]['content'])) {
                ...
                $valEnd = $this->phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_COMMA, \Dekode\GravityForms\Vendor\T_SEMICOLON), $valStart + 1, null, \false, null, \true);
                $val = $this->phpcsFile->getTokensAsString($valStart, $valEnd - $valStart);
                $val = $this->strip_quotes($val);
                $inst[$key][] = array($val, $token['line']);
            }
        } elseif (\in_array($token['code'], array(\T_CONSTANT_ENCAPSED_STRING, \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING), \true)) {
