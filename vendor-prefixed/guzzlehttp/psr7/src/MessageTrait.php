...
trait MessageTrait
{
    ...
    {
        if (!$this->stream) {
            $this->stream = \Dekode\GravityForms\Vendor\GuzzleHttp\Psr7\Utils::streamFor('');
        }
        return $this->stream;
    }
    public function withBody(\Dekode\GravityForms\Vendor\Psr\Http\Message\StreamInterface $body)
    {
        if ($body === $this->stream) {
            return $this;
        }
