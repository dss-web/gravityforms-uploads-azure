    ...
    private function createTokenMap()
    {
        ...
        for ($i = 0; $i < $this->numTokens; $i++) {
            /*
                Parenthesis mapping.
            */
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$parenthesisOpeners[$this->tokens[$i]['code']]) === \true) {
                ...
                if (isset($this->tokens[$i]['scope_condition']) === \true) {
                    ...
                    if ($currType === $tokenType && $isShared === \false && $opener === null) {
                        ...
                        if ($depth > 50) {
                            ...
                            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\TokenizerException('Maximum nesting level reached; file could not be processed');
                        }
                        $oldDepth = $depth;
                    ...
                    if (isset($this->tokens[$stackPtr]['parenthesis_closer']) === \true && $i < $this->tokens[$stackPtr]['parenthesis_closer']) {
                        ...
                        for ($x = $i - 1; $x > 0; $x--) {
                            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \true) {
                                ...
                                $disallowed = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$assignmentTokens;
                ...
                if ($tokenType === T_SEMICOLON && $opener === null && (isset($this->tokens[$stackPtr]['parenthesis_closer']) === \false || $i > $this->tokens[$stackPtr]['parenthesis_closer'])) {
                    ...
                    if ($tokenType === T_OPEN_PARENTHESIS) {
                        if (isset($this->tokens[$i]['parenthesis_owner']) === \true) {
                            ...
                            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$this->tokens[$owner]['code']]) === \true && isset($this->tokens[$i]['parenthesis_closer']) === \true) {
                        ...
                        if ($tokenType === T_OPEN_CURLY_BRACKET && $opener !== null) {
                            ...
                            if ($tokenType === T_CLOSE_CURLY_BRACKET && $ignore > 0) {
                                ...
                                if ($opener === null && isset($this->scopeOpeners[$currType]) === \true) {
                                    ...
                                    if ($this->tokens[$i]['line'] >= $startLine + 30 && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$i - 1]['code']]) === \false) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                if (empty($conditions) !== \true) {
                    ...
                    foreach ($conditions as $condition) {
                        $conditionString .= \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                    }
                    ...
                    if ($lastOpener !== null && $this->tokens[$lastOpener]['scope_closer'] === $i) {
                        foreach (\array_reverse($openers) as $opener) {
                            if ($this->tokens[$opener]['scope_closer'] === $i) {
                                ...
                                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                    echo \str_repeat("\t", $level + 1);
                                    echo '* token ' . \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($oldCondition) . ' removed from conditions array *' . \PHP_EOL;
                                }
                                ...
                                if ($condition !== $oldCondition) {
                                    if (isset($this->scopeOpeners[$oldCondition]['with'][$condition]) === \false) {
                                        ...
                                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                            $type = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($oldCondition);
                                            echo \str_repeat("\t", $level + 1);
