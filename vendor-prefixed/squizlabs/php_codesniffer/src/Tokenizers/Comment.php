...
class Comment
{
    ...
    public function tokenizeString($string, $eolChar, $stackPtr)
    {
        ...
        */
        ...
        $stackPtr++;
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($openTag);
        ...
        */
        ...
        foreach ($lines as $lineNum => $string) {
            ...
            if ($space !== null) {
                ...
                $stackPtr++;
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($space['content']);
            ...
            foreach ($lineTokens as $lineToken) {
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($lineToken['content']);
                    $type = $lineToken['type'];
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($closeTag['content']);
