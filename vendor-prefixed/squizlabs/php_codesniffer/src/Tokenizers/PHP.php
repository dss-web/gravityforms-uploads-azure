 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Tokenizers;

use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util;
class PHP extends \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Tokenizers\Tokenizer
{
    /**
    ...
    protected function tokenize($string)
    {
        ...
        $commentTokenizer = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Tokenizers\Comment();
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                if ($tokenIsArray === \true) {
                    $type = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($token[0]);
                    $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($token[1]);
                } else {
                    $newToken = self::resolveSimpleToken($token[0]);
                    $type = $newToken['type'];
                    $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($token[0]);
                }
                echo "\tProcess token ";
                if ($tokenIsArray === \true) {
                    echo "[{$stackPtr}]";
            ...
            //end if
            if ($newStackPtr > 0 && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$finalTokens[$newStackPtr - 1]['code']]) === \false) {
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_STRING || \preg_match('`^[a-zA-Z_\\x80-\\xff]`', $token[1]) === 1)) {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    if (\is_array($tokens[$i]) === \false || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$i][0]]) === \false) {
                        break;
                    }
                }
                if (isset($tokens[$i]) === \true && \is_array($tokens[$i]) === \false && $tokens[$i] === ':') {
                    ...
                    for ($j = $stackPtr - 1; $j > 0; $j--) {
                        if (\is_array($tokens[$j]) === \false || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$j][0]]) === \false) {
                            break;
                        }
                    }
            ...
            if (\PHP_VERSION_ID < 70000 && \PHP_VERSION_ID >= 50500 && $tokenIsArray === \true && $token[0] === \T_YIELD && isset($tokens[$stackPtr + 1]) === \true && isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE && $tokens[$stackPtr + 2][0] === \T_STRING && \strtolower($tokens[$stackPtr + 2][1]) === 'from') {
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    for ($i = $stackPtr + 1; $i <= $stackPtr + 2; $i++) {
                        $type = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($tokens[$i][0]);
                        $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($tokens[$i][1]);
            ...
            */
            if (\PHP_VERSION_ID < 70400 && ($tokenIsArray === \true && ($token[0] === \T_LNUMBER || $token[0] === \T_DNUMBER) && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_STRING && $tokens[$stackPtr + 1][1][0] === '_')) {
                ...
                $newToken['type'] = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($newType);
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_STRING && \strtolower($token[1]) === 'match' || $token[0] === \T_MATCH)) {
                ...
                for ($x = $stackPtr + 1; $x < $numTokens; $x++) {
                    if (isset($tokens[$x][0], \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \true) {
                        continue;
                    }
                    if ($tokens[$x] !== '(') {
            ...
            if ($tokenIsArray === \true && $token[0] === \T_DEFAULT) {
                if (isset($this->tstringContexts[$finalTokens[$lastNotEmptyToken]['code']]) === \false) {
                    for ($x = $stackPtr + 1; $x < $numTokens; $x++) {
                        ...
                        if (\is_array($tokens[$x]) === \false || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \false) {
            ...
            */
            if ($tokenIsArray === \false && $token[0] === '?') {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    ...
                    if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \true) {
                        continue;
                    }
                ...
                for ($i = $stackPtr - 1; $i >= 0; $i--) {
                    ...
                    if ($prevNonEmpty === null && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \false) {
                    ...
                    if ($tokenType === \T_FUNCTION || $tokenType === \T_FN || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$methodPrefixes[$tokenType]) === \true || $tokenType === \T_VAR) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            echo "\t\t* token {$stackPtr} changed from ? to T_NULLABLE" . \PHP_EOL;
                        }
                        $newToken['code'] = T_NULLABLE;
                    ...
                    if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \false) {
                        $lastSeenNonEmpty = $tokenType;
                    }
                }
                //end for
            ...
            */
            if ($stackPtr > 1 && (\is_array($tokens[$stackPtr - 1]) === \true && $tokens[$stackPtr - 1][0] === \T_PAAMAYIM_NEKUDOTAYIM) && $tokenIsArray === \true && $token[0] !== \T_STRING && $token[0] !== \T_VARIABLE && $token[0] !== T_DOLLAR && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$token[0]]) === \false) {
                $newToken = [];
                $newToken['code'] = \T_STRING;
                $newToken['type'] = 'T_STRING';
                $newToken['content'] = $token[1];
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_FUNCTION || $token[0] === \T_FN) && $finalTokens[$lastNotEmptyToken]['code'] !== \T_USE) {
                if ($token[0] === \T_FUNCTION) {
                    for ($x = $stackPtr + 1; $x < $numTokens; $x++) {
                        if (\is_array($tokens[$x]) === \false || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \false && $tokens[$x][1] !== '&') {
                            // Non-empty content.
                            break;
                        }
                    }
                    if ($x < $numTokens && \is_array($tokens[$x]) === \true && $tokens[$x][0] !== \T_STRING && $tokens[$x][0] !== \T_NAME_QUALIFIED) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            $oldType = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($tokens[$x][0]);
                ...
                */
                ...
                if ($parenthesisCloser !== \false) {
                    ...
                    if (isset($tokens[$x]) === \true && \is_array($tokens[$x]) === \false && $tokens[$x] === ':') {
                        ...
                        $allowed += \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
                        // Find the start of the return type.
                        for ($x += 1; $x < $numTokens; $x++) {
                            if (\is_array($tokens[$x]) === \true && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \true) {
            ...
            */
            if ($tokenIsArray === \true && \strpos($token[1], $this->eolChar) !== \false) {
                ...
                $numLines = \count($tokenLines);
                $newToken = ['type' => \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($token[0]), 'code' => $token[0], 'content' => ''];
                ...
                */
                if (isset($this->tokens[$i]['scope_opener']) === \true) {
                    for ($x = $i + 1; $x < $numTokens; $x++) {
                        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \false && $this->tokens[$x]['code'] !== T_BITWISE_AND) {
                            break;
                        }
                    }
                ...
                if ($this->tokens[$i]['code'] === \T_CLASS && isset($this->tokens[$i]['scope_opener']) === \true) {
                    ...
                    */
                    for ($x = $i + 1; $x < $numTokens; $x++) {
                        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \false) {
                            break;
                        }
                    }
                    ...
                    if ($this->tokens[$i]['code'] === \T_FN && isset($this->tokens[$i + 1]) === \true) {
                        ...
                        if (isset($this->tokens[$x]) === \true && $this->tokens[$x]['code'] === T_OPEN_PARENTHESIS) {
                            $ignore = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
                                ...
                                for ($scopeCloser = $arrow + 1; $scopeCloser < $numTokens; $scopeCloser++) {
                                    ...
                                    if (isset($this->tokens[$scopeCloser]['scope_closer'], $this->tokens[$scopeCloser]['scope_condition']) === \true && $scopeCloser === $this->tokens[$scopeCloser]['scope_closer'] && $this->tokens[$this->tokens[$scopeCloser]['scope_condition']]['code'] === \T_MATCH) {
                                        ...
                                        for ($lastNonEmpty = $scopeCloser - 1; $lastNonEmpty > $arrow; $lastNonEmpty--) {
                                            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$lastNonEmpty]['code']]) === \false) {
                        ...
                        if ($this->tokens[$i]['code'] === T_OPEN_SQUARE_BRACKET) {
                            ...
                            $allowed += \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$magicConstants;
                            for ($x = $i - 1; $x >= 0; $x--) {
                            ...
                            if ($this->tokens[$i]['code'] === \T_MATCH) {
                                if (isset($this->tokens[$i]['scope_opener'], $this->tokens[$i]['scope_closer']) === \false) {
                                    ...
                                    $searchFor += \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$scopeOpeners;
                                    for ($x = $this->tokens[$i]['scope_opener'] + 1; $x < $this->tokens[$i]['scope_closer']; $x++) {
                                ...
                                if ($this->tokens[$i]['code'] === T_BITWISE_OR) {
                                    ...
                                    */
                                    ...
                                    $typeTokenCount = 0;
                                    for ($x = $i + 1; $x < $numTokens; $x++) {
                                        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \true) {
                                            continue;
                                        }
                                    ...
                                    for ($x = $i - 1; $x >= 0; $x--) {
                                        ...
                                        if ($suspectedType === 'property or parameter' && (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$scopeModifiers[$this->tokens[$x]['code']]) === \true || $this->tokens[$x]['code'] === \T_VAR)) {
                                        ...
                                        if (isset($this->tokens[$last]['parenthesis_owner']) === \true && $this->tokens[$this->tokens[$last]['parenthesis_owner']]['code'] === \T_FUNCTION) {
                                            ...
                                            if (isset($this->tokens[$last]['parenthesis_opener']) === \true) {
                                                ...
                                                if ($this->tokens[$x]['code'] === \T_FN) {
                                                    for (--$x; $x >= 0; $x--) {
                                                        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \true || $this->tokens[$x]['code'] === T_BITWISE_AND) {
                                                            continue;
                                                        }
                                                        break;
                                                    }
                    ...
                    if ($oldCond === $this->tokens[$i]['code']) {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            ...
                            foreach ($oldConditions as $condition) {
                                $oldConds .= \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                            }
                            $oldConds = \rtrim($oldConds, ',');
                            $newConds = '';
                            foreach ($this->tokens[$x]['conditions'] as $condition) {
                                $newConds .= \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                            }
    ...
    public static function standardiseToken($token)
    {
        ...
        if ($token[0] === \T_STRING) {
            ...
            if ($token[0] === \T_CURLY_OPEN) {
                ...
                $newToken = ['code' => $token[0], 'type' => \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::tokenName($token[0])];
