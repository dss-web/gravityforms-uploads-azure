...
class JS extends \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Tokenizers\Tokenizer
{
    /**
     ...
     */
    public function __construct($content, \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config $config, $eolChar = '\\n')
    {
        if ($this->isMinifiedContent($content, $eolChar) === \true) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\TokenizerException('File appears to be minified and cannot be processed');
        }
    ...
    public function tokenize($string)
    {
        ...
        for ($i = 0; $i < $numChars; $i++) {
            $char = $chars[$i];
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($char);
                $bufferContent = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($buffer);
                if ($inString !== '') {
                    echo "\t";
                }
                if ($inComment !== '') {
            ...
            if ($inString === '' && $inComment === '' && $buffer !== '') {
                ...
                if (\trim($char) !== '' && \trim($buffer) === '') {
                    ...
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($buffer);
                ...
                if ($inString === $char) {
                    ...
                    if ($escapes === 0 || $escapes % 2 === 0) {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            ...
                            $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($buffer . $char);
            ...
            if ($buffer === $char && $char === '/' && $chars[$i + 1] !== '*') {
                ...
                if ($regex !== null) {
                    ...
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($regex['content']);
            ...
            if (isset($this->tokenValues[\strtolower($buffer)]) === \true && (\preg_match('|[a-zA-z0-9_]|', $char) === 0 || isset($chars[$i + 1]) === \false || \preg_match('|[a-zA-z0-9_]|', $chars[$i + 1]) === 0)) {
                ...
                if ($lookAheadLength > 0) {
                    ...
                    for ($x = 1; $x <= $lookAheadLength; $x++) {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($charBuffer);
                ...
                if (isset($this->tokenValues[\strtolower($char)]) === \true) {
                    ...
                    if ($newContent !== '') {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput(\substr($buffer, 0, -1));
                            echo "\t=> Added token T_STRING ({$content})" . \PHP_EOL;
                        }
                    }
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            if ($inComment === '' && \array_key_exists($buffer, $this->commentTokens) === \true) {
                ...
                if (isset($chars[$i - 2]) === \true && $chars[$i - 2] === '\\') {
                    ...
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        ...
                        $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($lastContent);
                    ...
                    for ($x = 0; $x < $lastNumChars; $x++) {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($lastChar);
                            echo "\t=> Added token {$value} ({$content})" . \PHP_EOL;
                        }
                    }
                } else {
            ...
            */
            ...
            if ($token['code'] === \T_OBJECT_OPERATOR) {
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$i]['code']]) === \true) {
                        continue;
                    }
    ...
    public function getRegexToken($char, $string, $chars, $tokens)
    {
        ...
        for ($prev = $numTokens - 1; $prev >= 0; $prev--) {
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$prev]['code']]) === \false) {
                break;
            }
        }
    ...
    public function processAdditional()
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($this->tokens[$i]['content']);
                ...
                if ($this->tokens[$i]['code'] === T_OPEN_CURLY_BRACKET && isset($this->tokens[$i]['scope_condition']) === \false && isset($this->tokens[$i]['bracket_closer']) === \true) {
                    ...
                    if ($condition === \T_CLASS) {
                        ...
                        for ($parenCloser = $i - 1; $parenCloser > 0; $parenCloser--) {
                            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$parenCloser]['code']]) === \false) {
                                break;
                            }
                        }
                        if ($this->tokens[$parenCloser]['code'] === T_CLOSE_PARENTHESIS) {
                            ...
                            for ($name = $parenOpener - 1; $name > 0; $name--) {
                                if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$name]['code']]) === \false) {
                                    break;
                                }
                            }
                    ...
                    if ($this->tokens[$i]['code'] === T_CLOSE_OBJECT) {
                        ...
                        if ($this->tokens[$i]['code'] === T_COLON) {
                            ...
                            for ($label = $i - 1; $label >= 0; $label--) {
                                if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$label]['code']]) === \false) {
                                    break;
                                }
                            }
