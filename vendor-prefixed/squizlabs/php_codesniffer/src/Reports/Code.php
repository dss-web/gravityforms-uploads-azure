...
class Code implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Reports\Report
{
    /**
        ...
        foreach ($report['messages'] as $line => $lineErrors) {
            ...
            if (isset($lineTokens[$startLine]) === \true) {
                for ($i = $lineTokens[$startLine]['start']; $i <= $lineTokens[$endLine]['end']; $i++) {
                    ...
                    $tokenContent = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($tokenContent, ["\r", "\n", "\t"]);
