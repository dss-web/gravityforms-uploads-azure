...
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\DeepExitException;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\DummyFile;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\FileList;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Cache;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards;
class Runner
{
    /**
    ...
    {
        try {
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Timing::startTiming();
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Runner::checkRequirements();
            if (\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_CBF') === \false) {
                \define('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_CBF', \false);
            }
            // Creating the Config object populates it with all required settings
            // based on the CLI arguments provided to the script and any config
            // values the user has set.
            $this->config = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config();
            ...
            if ($this->config->explain === \true) {
                ...
                foreach ($standards as $standard) {
                    ...
                    $ruleset = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Ruleset($this->config);
                    $ruleset->explain();
                }
                return 0;
            }
            ...
            if ($this->config->interactive === \false && ($toScreen === \false || $this->reporter->totalErrors + $this->reporter->totalWarnings === 0 && $this->config->showProgress === \true)) {
                \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Timing::printRunTime();
            }
        } catch (\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\DeepExitException $e) {
            echo $e->getMessage();
            return $e->getCode();
        }
        //end try
    ...
    {
        if (\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_CBF') === \false) {
            \define('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_CBF', \true);
        }
        try {
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Timing::startTiming();
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Runner::checkRequirements();
    ...
    public function checkRequirements()
    {
        ...
        if (\PHP_VERSION_ID < 50400) {
            ...
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\DeepExitException($error, 3);
        }
    ...
    public function init()
    {
        ...
        foreach ($this->config->standards as $standard) {
            if (\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards::isInstalledStandard($standard) === \false) {
                // They didn't select a valid coding standard, so help them
                // out by letting them know which standards are installed.
                $error = 'ERROR: the "' . $standard . '" coding standard is not installed. ';
                \ob_start();
                \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards::printInstalledStandards();
        ...
        if (\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_VERBOSITY') === \false) {
            \define('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_VERBOSITY', $this->config->verbosity);
        }
        // Create this class so it is autoloaded and sets up a bunch
        // of PHP_CodeSniffer-specific token type constants.
        $tokens = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens();
        // Allow autoloading of custom files inside installed standards.
        $installedStandards = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards::getInstalledStandardDetails();
        foreach ($installedStandards as $name => $details) {
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Autoload::addSearchPath($details['path'], $details['namespace']);
        }
        // The ruleset contains all the information about how the files
        // should be checked and/or fixed.
        try {
            $this->ruleset = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Ruleset($this->config);
    ...
    private function run()
    {
        ...
        $this->reporter = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Reporter($this->config);
        ...
        if ($this->config->stdin === \true) {
            ...
            $todo = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\FileList($this->config, $this->ruleset);
            $dummy = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\DummyFile($fileContents, $this->ruleset, $this->config);
            ...
            if ($this->config->cache === \true) {
                ...
                \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Cache::load($this->ruleset, $this->config);
                if (PHP_CODESNIFFER_VERBOSITY > 0) {
                    $size = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Cache::getSize();
        ...
        if ($this->config->parallel === 1) {
            ...
            foreach ($todo as $path => $file) {
                if ($file->ignored === \false) {
                    ...
                    if ($lastDir !== $currDir) {
                        if (PHP_CODESNIFFER_VERBOSITY > 0) {
                            echo 'Changing into directory ' . \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::stripBasepath($currDir, $this->config->basepath) . \PHP_EOL;
                        }
                        $lastDir = $currDir;
                    }
            ...
            for ($batch = 0; $batch < $this->config->parallel; $batch++) {
                ...
                $pid = \pcntl_fork();
                if ($pid === -1) {
                    throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('Failed to create child process');
                } else {
                    if ($pid !== 0) {
                        ...
                        if ($this->config->cache === \true) {
                            ...
                            foreach ($pathsProcessed as $path) {
                                $childCache[$path] = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Cache::get($path);
                            }
                            $output .= ";\n\$childCache = ";
            ...
            if ($success === \false) {
                throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('One or more child processes failed to run');
            }
        }
        //end if
        ...
        if ($this->config->cache === \true) {
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Cache::save();
        }
        $ignoreWarnings = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::getConfigData('ignore_warnings_on_exit');
        $ignoreErrors = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::getConfigData('ignore_errors_on_exit');
    ...
    public function handleErrors($code, $message, $file, $line)
    {
        ...
        throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException("{$message} in {$file} on line {$line}");
    }
    //end handleErrors()
    /**
            ...
            while ($numErrors !== 0) {
                ...
                switch ($input) {
                    case 's':
                        break 2;
                    case 'q':
                        throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\DeepExitException('', 0);
                    default:
                ...
                if ($res === $procData['pid']) {
                    if (\file_exists($procData['out']) === \true) {
                        ...
                        if (isset($childOutput) === \false) {
                            ...
                            $file = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\DummyFile('', $this->ruleset, $this->config);
                        ...
                        if (isset($childCache) === \true) {
                            foreach ($childCache as $path => $cache) {
                                \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Cache::set($path, $cache);
                            }
                        }
     ...
     * @return void
     */
    public function printProgress(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $file, $numFiles, $numProcessed)
    {
