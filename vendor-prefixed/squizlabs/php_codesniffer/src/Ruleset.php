 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer;
...
class Ruleset
{
     ...
     */
    public function __construct(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config $config)
    {
        $this->config = $config;
        ...
        foreach ($config->standards as $standard) {
            $installed = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards::getInstalledStandardPath($standard);
            if ($installed === null) {
                $standard = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($standard);
                if (\is_dir($standard) === \true && \is_file(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($standard . \DIRECTORY_SEPARATOR . 'ruleset.xml')) === \true) {
                    $standard = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($standard . \DIRECTORY_SEPARATOR . 'ruleset.xml');
                }
            } else {
                $standard = $installed;
            }
        ...
        foreach ($standardPaths as $standard) {
            ...
            if ($ruleset !== \false) {
                ...
                \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Autoload::addSearchPath(\dirname($standard), $namespace);
            }
            if (\defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS') === \true && empty($restrictions) === \false) {
                ...
                } catch (\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException $e) {
        ...
        if ($numSniffs === 0) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('No sniffs were registered');
        }
    }
    //end __construct()
    /**
    ...
    {
        $rulesetPath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($rulesetPath);
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            echo \str_repeat("\t", $depth);
            echo 'Processing ruleset ' . \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::stripBasepath($rulesetPath, $this->config->basepath) . \PHP_EOL;
        }
        ...
        if ($ruleset === \false) {
            ...
            \libxml_clear_errors();
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException($errorMsg);
        }
        ...
        if (\is_dir($sniffDir) === \true) {
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                echo \str_repeat("\t", $depth);
                echo "\tAdding sniff files from " . \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::stripBasepath($sniffDir, $this->config->basepath) . ' directory' . \PHP_EOL;
            }
        ...
        foreach ($ruleset->{'autoload'} as $autoload) {
            ...
            $relativePath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realPath(\dirname($rulesetPath) . \DIRECTORY_SEPARATOR . $autoloadPath);
            if ($relativePath !== \false && \is_file($relativePath) === \true) {
                $autoloadPath = $relativePath;
            } else {
                if (\is_file($autoloadPath) === \false) {
                    throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('The specified autoload file "' . $autoload . '" does not exist');
                }
            }
        ...
        foreach ($ruleset->{'config'} as $config) {
            ...
            \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::setConfigData((string) $config['name'], (string) $config['value'], \true);
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                echo \str_repeat("\t", $depth);
        ...
        if (empty($cliArgs) === \false) {
            ...
            $inPhar = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::isPharFile($rulesetDir);
            if ($inPhar === \false) {
                $currentDir = \getcwd();
                \chdir($rulesetDir);
            }
        ...
        foreach ($includedSniffs as $sniff) {
            if (\in_array($sniff, $excludedSniffs, \true) === \true) {
                ...
                $files[] = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($sniff);
            }
        }
        return $files;
    }
    ...
    private function expandSniffDirectory($directory, $depth = 0)
    {
        ...
        foreach ($di as $file) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                echo \str_repeat("\t", $depth);
                echo "\t\t=> " . \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::stripBasepath($path, $this->config->basepath) . \PHP_EOL;
            }
            $sniffs[] = $path;
        }
        //end foreach
    ...
    private function expandRulesetReference($ref, $rulesetDir, $depth = 0)
    {
        ...
        if (\substr($ref, 0, 1) === '.') {
            $realpath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($rulesetDir . '/' . $ref);
            if ($realpath !== \false) {
                $ref = $realpath;
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    echo \str_repeat("\t", $depth);
                    echo "\t\t=> " . \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::stripBasepath($ref, $this->config->basepath) . \PHP_EOL;
                }
            }
        }
        ...
        if (\substr($ref, 0, 2) === '~/') {
            $realpath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($ref);
            if ($realpath !== \false) {
                $ref = $realpath;
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    echo \str_repeat("\t", $depth);
        ...
        if (\is_file($ref) === \true) {
            ...
            $path = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards::getInstalledStandardPath($ref);
            if ($path !== null && \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::isPharFile($path) === \true && \strpos($path, 'ruleset.xml') === \false) {
            ...
            if ($path !== null) {
                ...
                if (\is_dir($ref) === \false) {
                    ...
                    $newRef = \false;
                    $stdPath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Standards::getInstalledStandardPath($stdName);
                    if ($stdPath !== null && $path !== '') {
                        if (\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::isPharFile($stdPath) === \true && \strpos($stdPath, 'ruleset.xml') === \false) {
                            // Phar files can only return the directory,
                            // since ruleset can be omitted if building one standard.
                            $newRef = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($stdPath . $path);
                        } else {
                            $newRef = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath(\dirname($stdPath) . $path);
                        }
                    }
                    if ($newRef === \false) {
                        ...
                        foreach ($this->rulesetDirs as $dir) {
                            ...
                            $newRef = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($dir . $path);
                            if ($newRef !== \false) {
                                $ref = $newRef;
                            }
                        }
        ...
        if (\is_dir($ref) === \true) {
            ...
            if (\is_file($ref) === \false) {
                ...
                throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException($error);
            }
            if (\substr($ref, -9) === 'Sniff.php') {
                // A single sniff.
                return [$ref];
        ...
        foreach ($todo as $code) {
            ...
            if (isset($rule->type) === \true && $this->shouldProcessElement($rule->type) === \true) {
                ...
                if ($type !== 'error' && $type !== 'warning') {
                    throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException("Message type \"{$type}\" is invalid; must be \"error\" or \"warning\"");
                }
    ...
    public function registerSniffs($files, $restrictions, $exclusions)
    {
        ...
        foreach ($files as $file) {
            ...
            $className = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Autoload::loadFile($file);
            $compareName = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::cleanSniffClass($className);
    ...
    public function populateTokenListeners()
    {
        ...
        foreach ($this->sniffs as $sniffClass => $sniffObject) {
            ...
            $sniffCode = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::getSniffCode($sniffClass);
            ...
            if (\is_array($tokens) === \false) {
                ...
                throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException($msg);
            }
            $ignorePatterns = [];
