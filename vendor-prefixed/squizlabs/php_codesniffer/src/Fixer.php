...
class Fixer
{
     ...
     * @return void
     */
    public function startFile(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile)
    {
    ...
    public function beginChangeset()
    {
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            if ($bt[1]['class'] === __CLASS__) {
                ...
                $sniff = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::getSniffCode($bt[1]['class']);
            }
            $line = $bt[0]['line'];
            @\ob_end_clean();
    ...
    public function rollbackChangeset()
    {
        ...
        if (empty($this->changeset) === \false) {
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                $sniff = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::getSniffCode($sniff);
    ...
    public function replaceToken($stackPtr, $content)
    {
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            $oldContent = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($this->tokens[$stackPtr]);
            $newContent = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($content);
            if (\trim($this->tokens[$stackPtr]) === '' && isset($this->tokens[$stackPtr + 1]) === \true) {
                // Add some context for whitespace only changes.
                $append = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($this->tokens[$stackPtr + 1]);
                $oldContent .= $append;
                $newContent .= $append;
            }
        }
    ...
    public function revertToken($stackPtr)
    {
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            $tokenLine = $tokens[$stackPtr]['line'];
            $oldContent = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($this->tokens[$stackPtr]);
            $newContent = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($this->fixedTokens[$stackPtr]);
