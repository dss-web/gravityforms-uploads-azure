 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files;

use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Ruleset;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Fixer;
...
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\TokenizerException;
class File
{
    /**
     ...
     * @return void
     */
    public function __construct($path, \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Ruleset $ruleset, \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config $config)
    {
        $this->path = $path;
        $this->ruleset = $ruleset;
        $this->config = $config;
        $this->fixer = new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Fixer();
        $parts = \explode('.', $path);
    ...
    public function setContent($content)
    {
        ...
        $this->tokens = [];
        try {
            $this->eolChar = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::detectLineEndings($content);
    ...
    public function process()
    {
        ...
        $foundCode = \false;
        $listenerIgnoreTo = [];
        $inTests = \defined('Dekode\\GravityForms\\Vendor\\PHP_CODESNIFFER_IN_TESTS');
        ...
        foreach ($this->tokens as $stackPtr => $token) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 2) {
                $type = $token['type'];
                $content = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::prepareForOutput($token['content']);
    ...
    public function parse()
    {
        ...
        } catch (\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\TokenizerException $e) {
            $this->ignored = \true;
    ...
    protected function addMessage($error, $message, $line, $column, $code, $data, $severity, $fixable)
    {
        ...
        if ($parts[0] === 'Internal') {
            // An internal message.
            $listenerCode = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::getSniffCode($this->activeListener);
            $sniffCode = $code;
    ...
    public function getDeclarationName($stackPtr)
    {
        ...
        if ($tokenCode !== \T_FUNCTION && $tokenCode !== \T_CLASS && $tokenCode !== \T_INTERFACE && $tokenCode !== \T_TRAIT) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('Token type "' . $this->tokens[$stackPtr]['type'] . '" is not T_FUNCTION, T_CLASS, T_INTERFACE or T_TRAIT');
        }
    ...
    public function getMethodParameters($stackPtr)
    {
        if ($this->tokens[$stackPtr]['code'] !== \T_FUNCTION && $this->tokens[$stackPtr]['code'] !== T_CLOSURE && $this->tokens[$stackPtr]['code'] !== \T_USE && $this->tokens[$stackPtr]['code'] !== \T_FN) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr must be of type T_FUNCTION or T_CLOSURE or T_USE or T_FN');
        }
        if ($this->tokens[$stackPtr]['code'] === \T_USE) {
            ...
            if ($opener === \false || isset($this->tokens[$opener]['parenthesis_owner']) === \true) {
                throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr was not a valid T_USE');
            }
        } else {
        ...
        for ($i = $paramStart; $i <= $closer; $i++) {
            ...
            switch ($this->tokens[$i]['code']) {
                ...
                case T_COMMA:
                    ...
                    break;
                case T_EQUAL:
                    $defaultStart = $this->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
                    $equalToken = $i;
                    break;
            }
            //end switch
    ...
    public function getMethodProperties($stackPtr)
    {
        if ($this->tokens[$stackPtr]['code'] !== \T_FUNCTION && $this->tokens[$stackPtr]['code'] !== T_CLOSURE && $this->tokens[$stackPtr]['code'] !== \T_FN) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr must be of type T_FUNCTION or T_CLOSURE or T_FN');
        }
    ...
    public function getMemberProperties($stackPtr)
    {
        if ($this->tokens[$stackPtr]['code'] !== \T_VARIABLE) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr must be of type T_VARIABLE');
        }
        ...
        if (isset($this->tokens[$ptr]) === \false || $this->tokens[$ptr]['code'] !== \T_CLASS && $this->tokens[$ptr]['code'] !== T_ANON_CLASS && $this->tokens[$ptr]['code'] !== \T_TRAIT) {
            if (isset($this->tokens[$ptr]) === \true && $this->tokens[$ptr]['code'] === \T_INTERFACE) {
                ...
                throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr is not a class member var');
            }
        }
        ...
        $valid += \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
        $scope = 'public';
        $scopeSpecified = \false;
        $isStatic = \false;
    ...
    public function getClassProperties($stackPtr)
    {
        if ($this->tokens[$stackPtr]['code'] !== \T_CLASS) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr must be of type T_CLASS');
        }
    ...
    public function isReference($stackPtr)
    {
        ...
        $tokenBefore = $this->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        ...
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$assignmentTokens[$this->tokens[$tokenBefore]['code']]) === \true) {
            // This is directly after an assignment. It's a reference. Even if
            // it is part of an operation, the other tests will handle it.
            return \true;
        }
        $tokenAfter = $this->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if ($this->tokens[$tokenBefore]['code'] === T_OPEN_PARENTHESIS || $this->tokens[$tokenBefore]['code'] === T_COMMA || $this->tokens[$tokenBefore]['code'] === T_OPEN_SHORT_ARRAY) {
            if ($this->tokens[$tokenAfter]['code'] === \T_VARIABLE) {
                ...
                $skip = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
                $skip[] = \T_NS_SEPARATOR;
                $skip[] = T_SELF;
                $skip[] = T_PARENT;
                $skip[] = \T_STATIC;
    ...
    public function getTokensAsString($start, $length, $origContent = \false)
    {
        if (\is_int($start) === \false || isset($this->tokens[$start]) === \false) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException('The $start position for getTokensAsString() must exist in the token stack');
        }
    ...
    {
        $startTokens = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$blockOpeners;
        ...
        if ($matchExpression !== \false) {
            ...
            if ($prevMatch <= $this->tokens[$matchExpression]['scope_opener']) {
                ...
                $next = $this->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $this->tokens[$matchExpression]['scope_opener'] + 1, null, \true);
                if ($next === \false) {
                    return $start;
                }
                return $next;
            ...
            if ($this->tokens[$prevMatch]['code'] === T_COMMA) {
                ...
                $next = $this->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, null, \true);
                return $next;
            }
        }
        //end if
        ...
        for ($i = $start; $i >= 0; $i--) {
            ...
            //end if
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$i]['code']]) === \false) {
                $lastNotEmpty = $i;
            }
        }
        //end for
    ...
    public function findEndOfStatement($start, $ignore = null)
    {
        ...
        for ($i = $start; $i < $this->numTokens; $i++) {
            ...
            if (isset($this->tokens[$i]['scope_closer']) === \true && ($i === $this->tokens[$i]['scope_opener'] || $i === $this->tokens[$i]['scope_condition'])) {
                ...
                if ($i === $start && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$this->tokens[$i]['code']]) === \true) {
