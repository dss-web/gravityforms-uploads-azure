...
class Standards
{
    ...
    public static function getInstalledStandardPaths()
    {
        ...
        $configPaths = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::getConfigData('installed_paths');
        if ($configPaths !== null) {
        ...
        foreach ($installedPaths as $installedPath) {
            if (\substr($installedPath, 0, 1) === '.') {
                $installedPath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realPath(__DIR__ . $ds . '..' . $ds . '..' . $ds . $installedPath);
    ...
    public static function getInstalledStandardPath($standard)
    {
        ...
        foreach ($installedPaths as $installedPath) {
            ...
            $path = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($standardPath . \DIRECTORY_SEPARATOR . 'ruleset.xml');
            if ($path !== \false && \is_file($path) === \true) {
                return $path;
            } else {
                if (\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::isPharFile($standardPath) === \true) {
                    $path = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::realpath($standardPath);
                    if ($path !== \false) {
                        return $path;
                    }
                }
