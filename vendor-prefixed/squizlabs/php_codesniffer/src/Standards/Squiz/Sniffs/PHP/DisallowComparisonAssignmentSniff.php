...
class DisallowComparisonAssignmentSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $stackPtr + 1; $i < $endStatement; $i++) {
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$comparisonTokens[$tokens[$i]['code']]) === \true && $tokens[$i]['code'] !== \T_COALESCE || $tokens[$i]['code'] === T_INLINE_THEN) {
                $error = 'The value of a comparison must not be assigned to a variable';
                $phpcsFile->addError($error, $stackPtr, 'AssignedComparison');
                break;
            }
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$booleanOperators[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === T_BOOLEAN_NOT) {
