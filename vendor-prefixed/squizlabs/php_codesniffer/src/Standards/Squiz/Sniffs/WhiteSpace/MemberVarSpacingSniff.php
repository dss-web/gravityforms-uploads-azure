 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\Squiz\Sniffs\WhiteSpace;

use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\AbstractVariableSniff;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens;
class MemberVarSpacingSniff extends \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\AbstractVariableSniff
{
    /**
     ...
     *                  pointer is reached.
     */
    protected function processMemberVar(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        $validPrefixes = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$methodPrefixes;
        $validPrefixes[] = \T_VAR;
        ...
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$prev]['code']]) === \true) {
            // Assume the comment belongs to the member var if it is on a line by itself.
            $prevContent = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $prev - 1, null, \true);
        ...
        if ($start === $startOfStatement) {
            // No comment found.
            $first = $phpcsFile->findFirstOnLine(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $start, \true);
            if ($first === \false) {
                $first = $start;
            }
        } else {
            if ($tokens[$start]['code'] === T_DOC_COMMENT_CLOSE_TAG) {
                ...
                $first = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $start - 1, null, \true);
                $first = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$commentTokens, $first + 1);
            }
        }
        ...
        if ($tokens[$prev]['code'] === T_OPEN_CURLY_BRACKET && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[$tokens[$tokens[$prev]['scope_condition']]['code']]) === \true) {
     ...
     * @return void
     */
    protected function processVariable(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        /*
     ...
     * @return void
     */
    protected function processVariableInString(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        /*
            We don't care about normal variables.
        */
