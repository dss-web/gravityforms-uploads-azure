...
class ControlStructureSpacingSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        for ($firstContent = $scopeOpener + 1; $firstContent < $phpcsFile->numTokens; $firstContent++) {
            ...
            if ($tokens[$firstContent]['line'] === $tokens[$scopeOpener]['line'] && (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$code]) === \true || $code === \T_CLOSE_TAG)) {
                continue;
            }
            break;
        }
        ...
        if ($firstContent !== $scopeCloser) {
            ...
            $lastNonEmptyContent = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $scopeCloser - 1, null, \true);
        ...
        if ($tokens[$stackPtr]['code'] === \T_MATCH) {
            ...
            $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $scopeCloser + 1, null, \true);
        ...
        if ($tokens[$trailingContent]['code'] === \T_COMMENT || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$trailingContent]['code']]) === \true) {
            // Special exception for code where the comment about
            // an ELSE or ELSEIF is written between the control structures.
            $nextCode = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $scopeCloser + 1, null, \true);
        ...
        if (isset($tokens[$trailingContent]['scope_condition']) === \true && $tokens[$trailingContent]['scope_condition'] !== $trailingContent && isset($tokens[$trailingContent]['scope_opener']) === \true && $tokens[$trailingContent]['scope_opener'] !== $trailingContent) {
            ...
            if ($tokens[$trailingContent]['code'] !== \T_ELSE && $tokens[$trailingContent]['code'] !== \T_ELSEIF && $tokens[$trailingContent]['code'] !== \T_CATCH && $tokens[$trailingContent]['code'] !== \T_FINALLY && $tokens[$trailingContent]['line'] === $tokens[$scopeCloser]['line'] + 1) {
                ...
                if ($fix === \true) {
                    $trailingContent = $phpcsFile->findNext(\T_WHITESPACE, $scopeCloser + 1, null, \true);
                    if (($tokens[$trailingContent]['code'] === \T_COMMENT || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$trailingContent]['code']]) === \true) && $tokens[$trailingContent]['line'] === $tokens[$scopeCloser]['line']) {
