 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\MySource\Sniffs\Objects;
...
class CreateWidgetTypeCallbackSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
        ...
        */
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if ($tokens[$i + 1]['code'] !== \T_OBJECT_OPERATOR || $tokens[$i + 2]['content'] !== 'call' || $tokens[$i + 3]['code'] !== T_OPEN_PARENTHESIS) {
                ...
                $prev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $bracket - 1, null, \true);
            ...
            for ($next = $endBracket; $next <= $end; $next++) {
                ...
                if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$next]['code']]) === \true) {
                    continue;
                }
