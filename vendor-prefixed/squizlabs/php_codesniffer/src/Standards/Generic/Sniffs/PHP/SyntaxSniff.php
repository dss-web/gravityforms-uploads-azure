...
class SyntaxSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        if ($this->phpPath === null) {
            $this->phpPath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::getExecutablePath('php');
        }
        $fileName = \escapeshellarg($phpcsFile->getFilename());
        $cmd = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::escapeshellcmd($this->phpPath) . " -l -d display_errors=1 -d error_prepend_string='' {$fileName} 2>&1";
