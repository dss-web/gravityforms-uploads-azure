 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\Generic\Sniffs\WhiteSpace;
...
class ScopeIndentSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        $debug = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::getConfigData('scope_indent_debug');
        if ($debug !== null) {
        ...
        for ($i = $stackPtr + 1; $i < $phpcsFile->numTokens; $i++) {
            ...
            */
            ...
            //end if
            if ($checkToken !== null && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$tokens[$checkToken]['code']]) === \true && \in_array($tokens[$checkToken]['code'], $this->nonIndentingScopes, \true) === \false && isset($tokens[$checkToken]['scope_opener']) === \true) {
                $exact = \true;
            ...
            if ($checkToken !== null && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$methodPrefixes[$tokens[$checkToken]['code']]) === \true && $tokens[$checkToken + 1]['code'] !== \T_DOUBLE_COLON) {
                $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $checkToken + 1, null, \true);
            ...
            */
            ...
            if ($tokens[$i]['code'] === \T_START_HEREDOC || $tokens[$i]['code'] === T_START_NOWDOC) {
                ...
                $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
            ...
            if (isset($tokens[$i]['scope_condition']) === \true && isset($tokens[$i]['scope_opener']) === \true && $tokens[$i]['scope_opener'] === $i) {
                ...
                if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$condition]) === \true && \in_array($condition, $this->nonIndentingScopes, \true) === \false) {
                    if ($this->debug === \true) {
                        $line = $tokens[$i]['line'];
                        $type = $tokens[$tokens[$i]['scope_condition']]['type'];
            ...
            if (isset($tokens[$i]['scope_condition']) === \true && $tokens[$i]['scope_closer'] === $i && ($tokens[$tokens[$i]['scope_condition']]['code'] === T_CLOSURE || $tokens[$tokens[$i]['scope_condition']]['code'] === T_ANON_CLASS)) {
                ...
                if ($parens > $object && $parens > $condition) {
                    ...
                    $prev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $parens - 1, null, \true);
                    $object = 0;
                    $condition = 0;
                } else {
     ...
     * @return bool
     */
    protected function adjustIndent(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, $length, $change)
    {
        $tokens = $phpcsFile->getTokens();
