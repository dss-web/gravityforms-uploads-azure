 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\PSR2\Sniffs\ControlStructures;
...
class SwitchDeclarationSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
        ...
        while (($nextCase = $this->findNextCase($phpcsFile, $nextCase + 1, $switch['scope_closer'])) !== \false) {
            ...
            if ($tokens[$opener]['code'] === T_COLON) {
                ...
                for ($next = $opener + 1; $next < $nextCloser; $next++) {
                    if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$next]['code']]) === \false || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$next]['code']]) === \true && $tokens[$next]['line'] !== $tokens[$opener]['line']) {
                        break;
                    }
                }
            ...
            if ($tokens[$nextCode]['code'] !== \T_CASE && $tokens[$nextCode]['code'] !== \T_DEFAULT) {
                ...
                if ($nextCode !== \false) {
                    ...
                    if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$prevCode]['code']]) === \false && $this->findNestedTerminator($phpcsFile, $opener + 1, $nextCode) === \false) {
    ...
    private function findNestedTerminator($phpcsFile, $stackPtr, $end)
    {
        ...
        if ($tokens[$lastToken]['code'] === T_CLOSE_CURLY_BRACKET) {
            ...
            do {
                ...
                $prevToken = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $scopeOpener - 1, $stackPtr, \true);
                if ($prevToken === \false) {
                    return \false;
                }
                ...
                if ($tokens[$prevToken]['code'] === \T_IF) {
                    ...
                    if ($tokens[$prevToken]['code'] === \T_ELSEIF || $tokens[$prevToken]['code'] === \T_ELSE) {
                        ...
                        $currentCloser = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $prevToken - 1, $stackPtr, \true);
                                ...
                                if ($tokens[$prevToken]['code'] === \T_CATCH) {
                                    ...
                                    if ($tokens[$prevToken]['code'] === \T_SWITCH) {
                                        ...
                                        while (($nextCase = $this->findNextCase($phpcsFile, $nextCase + 1, $endOfSwitch)) !== \false) {
                                            ...
                                            $opener = $tokens[$nextCase]['scope_opener'];
                                            $nextCode = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $opener + 1, $endOfSwitch, \true);
                                            if ($tokens[$nextCode]['code'] === \T_CASE || $tokens[$nextCode]['code'] === \T_DEFAULT) {
