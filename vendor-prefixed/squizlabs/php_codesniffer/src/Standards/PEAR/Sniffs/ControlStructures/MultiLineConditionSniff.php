 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\PEAR\Sniffs\ControlStructures;
...
class MultiLineConditionSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $openBracket + 1; $i <= $closeBracket; $i++) {
            if ($i === $closeBracket && $tokens[$openBracket]['line'] !== $tokens[$i]['line']) {
                ...
                if ($tokens[$prev]['line'] === $tokens[$i]['line']) {
                    ...
                    if ($fix === \true) {
                        ...
                        if ($tokens[$next]['code'] !== \T_COMMENT && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$next]['code']]) === \false) {
                            $phpcsFile->fixer->addNewlineBefore($closeBracket);
                        } else {
                            $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next + 1, null, \true);
            ...
            if ($tokens[$i]['line'] !== $prevLine) {
                ...
                //end if
                if ($tokens[$i]['code'] === \T_COMMENT || isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$i]['code']]) === \true) {
                ...
                $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i, null, \true);
                if ($next !== $closeBracket && $tokens[$next]['line'] === $tokens[$i]['line']) {
                    if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$booleanOperators[$tokens[$next]['code']]) === \false) {
                        $prev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i - 1, $openBracket, \true);
                        $fixable = \true;
                        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$booleanOperators[$tokens[$prev]['code']]) === \false && $phpcsFile->findNext(\T_WHITESPACE, $prev + 1, $next, \true) !== \false) {
                        ...
                        if ($fixable === \false) {
                            ...
                            if ($fix === \true) {
                                if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$booleanOperators[$tokens[$prev]['code']]) === \true) {
                                    $phpcsFile->fixer->beginChangeset();
