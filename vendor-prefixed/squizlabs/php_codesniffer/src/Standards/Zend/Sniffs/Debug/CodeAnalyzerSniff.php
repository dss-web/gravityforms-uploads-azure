 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\Zend\Sniffs\Debug;
...
class CodeAnalyzerSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        $analyzerPath = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Config::getExecutablePath('zend_ca');
        ...
        $cmd = \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Common::escapeshellcmd($analyzerPath) . ' ' . \escapeshellarg($fileName) . ' 2>&1';
        ...
        if (\is_numeric($exitCode) === \true && $exitCode > 0) {
            ...
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Exceptions\RuntimeException("Failed invoking ZendCodeAnalyzer, exitcode was [{$exitCode}], retval was [{$retval}], output was [{$msg}]");
        }
        if (\is_array($output) === \true) {
