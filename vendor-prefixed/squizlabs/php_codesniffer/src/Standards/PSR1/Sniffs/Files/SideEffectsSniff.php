 ...
 */
namespace Dekode\GravityForms\Vendor\PHP_CodeSniffer\Standards\PSR1\Sniffs\Files;
...
class SideEffectsSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    private function searchForConflict($phpcsFile, $start, $end, $tokens)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$booleanOperators[$tokens[$i]['code']]) === \true) {
                continue;
            }
            ...
            if ($tokens[$i]['code'] === \T_NAMESPACE || $tokens[$i]['code'] === \T_USE || $tokens[$i]['code'] === \T_DECLARE || $tokens[$i]['code'] === \T_CONST) {
                if (isset($tokens[$i]['scope_opener']) === \true) {
                    ...
                    if ($tokens[$i]['code'] === \T_ENDDECLARE) {
                        $semicolon = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$methodPrefixes[$tokens[$i]['code']]) === \true) {
                continue;
            }
            // Ignore anon classes.
            ...
            if (isset($symbols[$tokens[$i]['code']]) === \true && isset($tokens[$i]['scope_closer']) === \true) {
                ...
                if ($tokens[$i]['code'] === \T_STRING && \strtolower($tokens[$i]['content']) === 'define') {
                    $prev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i - 1, null, \true);
            ...
            if ($tokens[$i]['code'] === \T_STRING && \strtolower($tokens[$i]['content']) === 'defined') {
                $openBracket = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
