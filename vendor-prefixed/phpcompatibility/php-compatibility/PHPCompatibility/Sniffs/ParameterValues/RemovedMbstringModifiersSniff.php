 ...
 */
class RemovedMbstringModifiersSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\AbstractFunctionCallParameterSniff
{
    /**
    ...
    public function processParameters(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        $stringToken = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$stringTokens, $optionsParam['start'], $optionsParam['end'] + 1);
        ...
        for ($i = $stringToken; $i <= $optionsParam['end']; $i++) {
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$stringTokens[$tokens[$i]['code']]) === \false) {
                continue;
            }
