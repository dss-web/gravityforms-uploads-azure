 ...
 */
class RemovedPCREModifiersSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\AbstractFunctionCallParameterSniff
{
    /**
    ...
    public function processParameters(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $firstParam['start'], $firstParam['end'] + 1, \true);
        if ($nextNonEmpty !== \false && ($tokens[$nextNonEmpty]['code'] === \T_ARRAY || $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY)) {
     ...
     * @return void
     */
    protected function processRegexPattern($pattern, \Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $functionName)
    {
        $tokens = $phpcsFile->getTokens();
        /*
        ...
        for ($i = $pattern['start']; $i <= $pattern['end']; $i++) {
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$stringTokens[$tokens[$i]['code']]) === \true) {
                $content = $this->stripQuotes($tokens[$i]['content']);
                if ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING) {
     ...
     * @return void
     */
    protected function examineModifiers(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $functionName, $modifiers)
    {
