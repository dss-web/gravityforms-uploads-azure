 ...
 * @since 9.3.0
 */
class RemovedImplodeFlexibleParamOrderSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\AbstractFunctionCallParameterSniff
{
    /**
    ...
    public function processParameters(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        $isOnlyText = \true;
        $firstNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $start, $end, \true);
        if ($firstNonEmpty === \false) {
            // Parse error. Shouldn't be possible.
            return;
        }
        if ($tokens[$firstNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS) {
            $start = $firstNonEmpty + 1;
            $end = $tokens[$firstNonEmpty]['parenthesis_closer'];
        }
        $hasTernary = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_INLINE_THEN, $start, $end);
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens[$tokenCode])) {
                continue;
            }
            if ($tokenCode === \T_STRING && isset($this->constantStrings[$tokens[$i]['content']])) {
                continue;
            }
            if ($hasTernary !== \false && $tokenCode === \Dekode\GravityForms\Vendor\T_INLINE_ELSE) {
                continue;
            }
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$stringTokens[$tokenCode]) === \false) {
                $isOnlyText = \false;
            }
            if ($tokenCode === \T_ARRAY || $tokenCode === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY || $tokenCode === \T_ARRAY_CAST) {
            ...
            if ($tokenCode === \T_STRING) {
                 ...
                 */
                $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i + 1, $end, \true);
                ...
                $prevNonEmpty = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i - 1, $start, \true);
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if ($tokenCode === \T_CONSTANT_ENCAPSED_STRING || $tokenCode === \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING || $tokenCode === \Dekode\GravityForms\Vendor\T_HEREDOC || $tokenCode === \Dekode\GravityForms\Vendor\T_NOWDOC) {
                $this->throwNotice($phpcsFile, $stackPtr, $functionName);
                return;
            }
        }
     ...
     * @return void
     */
    protected function throwNotice(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $functionName)
    {
