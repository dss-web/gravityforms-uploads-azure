 ...
 * @since 9.2.0
 */
class RemovedTernaryAssociativitySniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $endOfStatement = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::findEndOfStatement($phpcsFile, $stackPtr);
        if ($tokens[$endOfStatement]['code'] !== \Dekode\GravityForms\Vendor\T_SEMICOLON && $tokens[$endOfStatement]['code'] !== \Dekode\GravityForms\Vendor\T_COLON && $tokens[$endOfStatement]['code'] !== \Dekode\GravityForms\Vendor\T_COMMA && $tokens[$endOfStatement]['code'] !== \T_DOUBLE_ARROW && $tokens[$endOfStatement]['code'] !== \T_OPEN_TAG && $tokens[$endOfStatement]['code'] !== \T_CLOSE_TAG) {
        ...
        for ($i = $stackPtr; $i < $endOfStatement; $i++) {
            if (($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY || $tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET || $tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET) && isset($tokens[$i]['bracket_closer'])) {
                // Skip over short arrays, array access keys and curlies.
                $i = $tokens[$i]['bracket_closer'];
                continue;
            }
            if ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS && isset($tokens[$i]['parenthesis_closer'])) {
                // Skip over anything between parentheses.
                $i = $tokens[$i]['parenthesis_closer'];
                continue;
            }
            // Check for operators with lower operator precedence.
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$i]['code']]) || isset($this->tokensWithLowerPrecedence[$tokens[$i]['code']])) {
                break;
            }
            if ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_INLINE_THEN) {
                ++$ternaryCount;
            ...
            if ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_INLINE_ELSE) {
