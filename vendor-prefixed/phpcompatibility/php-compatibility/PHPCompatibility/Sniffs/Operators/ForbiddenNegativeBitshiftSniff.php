 ...
 * @since 7.0.0
 */
class ForbiddenNegativeBitshiftSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
     ...
     * @var array
     */
    private $inclusiveStopPoints = array(\Dekode\GravityForms\Vendor\T_COLON => \true, \Dekode\GravityForms\Vendor\T_COMMA => \true, \T_DOUBLE_ARROW => \true, \Dekode\GravityForms\Vendor\T_SEMICOLON => \true);
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $start = $stackPtr + 1;
        $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $start, null, \true);
        if ($next !== \false && $tokens[$next]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS) {
            $start = $next + 1;
        }
        $end = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::findEndOfStatement($phpcsFile, $start);
