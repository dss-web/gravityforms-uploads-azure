...
use Dekode\GravityForms\Vendor\PHPCompatibility\AbstractRemovedFeatureSniff;
 ...
 */
class RemovedPredefinedGlobalVariablesSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\AbstractRemovedFeatureSniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->inClassScope($phpcsFile, $stackPtr, \false) === \true) {
            $prevToken = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
     ...
     * @return bool
     */
    private function isTargetPHPErrormsgVar(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, array $tokens)
    {
        $scopeStart = 0;
        /*
         * If the variable is detected within the scope of a function/closure, limit the checking.
         */
        $function = $phpcsFile->getCondition($stackPtr, \Dekode\GravityForms\Vendor\T_CLOSURE);
        if ($function === \false) {
        ...
        if ($function === \false && isset($tokens[$stackPtr]['nested_parenthesis']) === \true) {
            ...
            if (isset($tokens[$parenthesisCloser]['parenthesis_owner']) && ($tokens[$tokens[$parenthesisCloser]['parenthesis_owner']]['code'] === \T_FUNCTION || $tokens[$tokens[$parenthesisCloser]['parenthesis_owner']]['code'] === \Dekode\GravityForms\Vendor\T_CLOSURE)) {
        ...
        if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET) {
        ...
        if ($nextNonEmpty !== \false && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
            return \false;
        }
        // Is this a function param shadowing the PHP native one ?
        if ($function !== \false) {
            $parameters = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getMethodParameters($phpcsFile, $function);
        ...
        for ($i = $stackPtr - 1; $i >= $scopeStart; $i--) {
            if ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_CLOSE_CURLY_BRACKET && isset($tokens[$i]['scope_condition']) && isset($skipPast[$tokens[$tokens[$i]['scope_condition']]['type']])) {
            ...
            $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i + 1, null, \true);
            if ($nextNonEmpty !== \false && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
                return \false;
            }
        }
