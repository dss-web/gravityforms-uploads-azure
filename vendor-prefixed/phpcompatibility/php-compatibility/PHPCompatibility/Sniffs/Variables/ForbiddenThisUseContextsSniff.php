 ...
 * @since 9.1.0
 */
class ForbiddenThisUseContextsSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
     * OO scope tokens.
     *
    ...
    public function register()
    {
        if (\defined('Dekode\\GravityForms\\Vendor\\T_ANON_CLASS')) {
            $this->ooScopeTokens['T_ANON_CLASS'] = \Dekode\GravityForms\Vendor\T_ANON_CLASS;
        }
        $this->skipOverScopes += $this->ooScopeTokens;
        return array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE, \T_GLOBAL, \T_CATCH, \T_FOREACH, \T_UNSET);
    }
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        switch ($tokens[$stackPtr]['code']) {
            case \T_FUNCTION:
                ...
                break;
            case \Dekode\GravityForms\Vendor\T_CLOSURE:
                $this->isThisUsedAsParameter($phpcsFile, $stackPtr);
                break;
            case \T_GLOBAL:
                /*
            ...
            case \T_FOREACH:
                ...
                $afterThis = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $valueVarPtr + 1, $tokens[$stackPtr]['parenthesis_closer'], \true);
            ...
            case \T_UNSET:
                 ...
                 */
                $openParenthesis = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true);
                if ($openParenthesis === \false || $tokens[$openParenthesis]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS || isset($tokens[$openParenthesis]['parenthesis_closer']) === \false) {
                    return;
                }
                for ($i = $openParenthesis + 1; $i < $tokens[$openParenthesis]['parenthesis_closer']; $i++) {
                    ...
                    $afterThis = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i + 1, $tokens[$openParenthesis]['parenthesis_closer'], \true);
                    if ($afterThis !== \false && ($tokens[$afterThis]['code'] === \T_OBJECT_OPERATOR || $tokens[$afterThis]['code'] === \T_DOUBLE_COLON || $tokens[$afterThis]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET)) {
                        $i = $afterThis;
                        continue;
                    }
     ...
     * @return void
     */
    protected function isThisUsedAsParameter(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
     ...
     * @return void
     */
    protected function isThisUsedOutsideObjectContext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        for ($i = $tokens[$stackPtr]['scope_opener'] + 1; $i < $tokens[$stackPtr]['scope_closer']; $i++) {
            ...
            if (isset($tokens[$i]['nested_parenthesis']) === \true) {
                ...
                $previousNonEmpty = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $lastOpenParenthesis - 1, null, \true, null, \true);
