 ...
 * @since 7.0.0
 */
class NewClosureSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    {
        return array(\Dekode\GravityForms\Vendor\T_CLOSURE);
    }
    /**
     ...
     * @return bool
     */
    protected function isClosureStatic(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
     ...
     *                   $this is not used.
     */
    protected function findThisUsageInClosure(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $startToken, $endToken)
    {
     ...
     *                   they are not used.
     */
    protected function findClassRefUsageInClosure(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $startToken, $endToken)
    {
        // Make sure the $startToken is valid.
        if ($startToken >= $endToken) {
            return \false;
        ...
        $tokens = $phpcsFile->getTokens();
        $classRef = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_SELF, \Dekode\GravityForms\Vendor\T_PARENT, \T_STATIC), $startToken, $endToken);
        if ($classRef === \false || $tokens[$classRef]['code'] !== \T_STATIC) {
            return $classRef;
        }
        // T_STATIC, make sure it is used as a class reference.
        $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $classRef + 1, $endToken, \true);
