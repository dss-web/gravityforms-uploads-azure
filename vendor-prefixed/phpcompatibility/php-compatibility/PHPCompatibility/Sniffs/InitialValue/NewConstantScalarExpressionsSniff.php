 ...
 */
namespace Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\InitialValue;
 ...
 * @since 8.2.0
 */
class NewConstantScalarExpressionsSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
     * Error message.
     *
    ...
    protected $safeOperands = array(
        ...
        \Dekode\GravityForms\Vendor\T_TRUE => \Dekode\GravityForms\Vendor\T_TRUE,
        \Dekode\GravityForms\Vendor\T_FALSE => \Dekode\GravityForms\Vendor\T_FALSE,
        \Dekode\GravityForms\Vendor\T_NULL => \Dekode\GravityForms\Vendor\T_NULL,
        \T_LINE => \T_LINE,
        \T_FILE => \T_FILE,
        \T_DIR => \T_DIR,
        \T_FUNC_C => \T_FUNC_C,
    ...
    public function register()
    {
        ...
        $this->setProperties();
        return array(\T_CONST, \T_VARIABLE, \T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE, \T_STATIC);
    }
    /**
    ...
    {
        $this->safeOperands += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$heredocTokens;
        $this->safeOperands += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens;
    }
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        switch ($tokens[$stackPtr]['type']) {
            case 'T_FUNCTION':
            case 'T_CLOSURE':
                $params = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getMethodParameters($phpcsFile, $stackPtr);
                if (empty($params)) {
                    // No parameters.
                    return;
                }
                ...
                foreach ($params as $param) {
                    ...
                    $end = $param['token'];
                    while (($end = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_COMMA, \Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS), $end + 1, $closer + 1)) !== \false) {
                        ...
                        if ($tokens[$end]['code'] === \Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS && $end !== $closer) {
                            continue;
                        }
            ...
            case 'T_CONST':
                ...
                if ($tokens[$stackPtr]['code'] === \T_STATIC) {
                    $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
                ...
                $start = $stackPtr;
                $end = $stackPtr;
                while (($end = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_COMMA, \Dekode\GravityForms\Vendor\T_SEMICOLON, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, \T_CLOSE_TAG), $end + 1, $endOfStatement + 1)) !== \false) {
                    $maybeSkipTo = $this->isRealEndOfDeclaration($tokens, $end, $targetNestingLevel);
                    if ($maybeSkipTo !== \true) {
                        $end = $maybeSkipTo;
                        continue;
                    }
                    $start = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $start + 1, $end, \true);
     ...
     * @return bool
     */
    protected function isValidAssignment(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $end)
    {
        $tokens = $phpcsFile->getTokens();
        $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, $end, \true);
        if ($next === \false || $tokens[$next]['code'] !== \Dekode\GravityForms\Vendor\T_EQUAL) {
            // No value assigned.
            return \true;
        }
     ...
     * @return bool
     */
    protected function isStaticValue(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $tokens, $start, $end, $nestedArrays = 0)
    {
        ...
        switch ($tokens[$nextNonSimple]['code']) {
            case \Dekode\GravityForms\Vendor\T_MINUS:
            case \Dekode\GravityForms\Vendor\T_PLUS:
                ...
                return \false;
            case \T_NAMESPACE:
            case \Dekode\GravityForms\Vendor\T_PARENT:
            case \Dekode\GravityForms\Vendor\T_SELF:
            case \T_DOUBLE_COLON:
                $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $nextNonSimple + 1, $end + 1, \true);
                if ($tokens[$nextNonSimple]['code'] === \T_NAMESPACE) {
                ...
                } elseif ($tokens[$nextNonSimple]['code'] === \Dekode\GravityForms\Vendor\T_PARENT || $tokens[$nextNonSimple]['code'] === \Dekode\GravityForms\Vendor\T_SELF) {
                    ...
                    $prevNonEmpty = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $nextNonSimple - 1, null, \true);
            ...
            case \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY:
                ...
                if (empty($arrayItems) === \false) {
                    foreach ($arrayItems as $item) {
                        ...
                        $doubleArrow = \false;
                        $maybeDoubleArrow = $phpcsFile->findNext(array(\T_DOUBLE_ARROW, \T_ARRAY, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY), $item['start'], $item['end'] + 1);
                ...
                if ($tokens[$nextNonSimple]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY && isset($tokens[$nextNonSimple]['bracket_closer']) === \true) {
                    $closer = $tokens[$nextNonSimple]['bracket_closer'];
                } else {
                    $maybeOpener = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $nextNonSimple + 1, $end + 1, \true);
                    if ($tokens[$maybeOpener]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS) {
                        $opener = $maybeOpener;
     ...
     * @return void
     */
    protected function throwError(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $type, $content = '')
    {
    ...
    private function isRealEndOfDeclaration($tokens, $endPtr, $targetLevel)
    {
        ...
        if ($tokens[$endPtr]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY && (isset($tokens[$endPtr]['bracket_opener']) && $tokens[$endPtr]['bracket_opener'] === $endPtr) && isset($tokens[$endPtr]['bracket_closer'])) {
            // Skip forward to the end of the short array definition.
            return $tokens[$endPtr]['bracket_closer'];
        }
        // Skip past comma's at a lower nesting level.
        if ($tokens[$endPtr]['code'] === \Dekode\GravityForms\Vendor\T_COMMA) {
