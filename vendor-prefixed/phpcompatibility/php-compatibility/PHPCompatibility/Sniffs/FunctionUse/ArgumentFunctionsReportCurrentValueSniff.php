 ...
 */
namespace Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\FunctionUse;
 ...
 * @since 9.1.0
 */
class ArgumentFunctionsReportCurrentValueSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
     ...
     * @var array
     */
    private $ignoreForStartOfStatement = array(\Dekode\GravityForms\Vendor\T_COMMA, \T_DOUBLE_ARROW, \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET, \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS);
    /**
    ...
    {
        return array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE);
    }
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $scopeOpener + 1; $i < $scopeCloser; $i++) {
             ...
             */
            $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i + 1, null, \true);
            if ($next === \false || $tokens[$next]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS) {
                // Live coding, parse error or not a function call.
                continue;
            }
            $prev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i - 1, null, \true);
            if ($prev !== \false) {
                ...
                if ($tokens[$prev]['code'] === \T_NS_SEPARATOR) {
                    $pprev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $prev - 1, null, \true);
            ...
            if ($foundFunctionName !== 'func_get_args') {
                 ...
                 *  Should be exceptionally rare and can - if needs be - be addressed at a later stage.}
                 */
                if ($prev !== \false && $tokens[$prev]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS) {
                    $maybeFunctionCall = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $prev - 1, null, \true);
            ...
            if ($foundFunctionName === 'debug_backtrace' && isset($tokens[$next]['parenthesis_closer'])) {
                $afterParenthesis = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $tokens[$next]['parenthesis_closer'] + 1, null, \true);
                if ($tokens[$afterParenthesis]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET && isset($tokens[$afterParenthesis]['bracket_closer'])) {
                    $afterStackFrame = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $tokens[$afterParenthesis]['bracket_closer'] + 1, null, \true);
                    if ($tokens[$afterStackFrame]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET && isset($tokens[$afterStackFrame]['bracket_closer'])) {
             ...
             * `$param = func_get_args();`.
             */
            $startOfStatement = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::findStartOfStatement($phpcsFile, $i, $this->ignoreForStartOfStatement);
            /*
            ...
            for ($j = $scopeOpener + 1; $j < $startOfStatement; $j++) {
                ...
                $beforeVar = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $j - 1, null, \true);
                ...
                $afterVar = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $j + 1, null, \true);
                if ($afterVar === \false) {
                ...
                if ($tokens[$afterVar]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET && isset($tokens[$afterVar]['bracket_closer'])) {
                    // Skip past array access on the variable.
                    while (($afterVar = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $tokens[$afterVar]['bracket_closer'] + 1, null, \true)) !== \false) {
                        if ($tokens[$afterVar]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET || isset($tokens[$afterVar]['bracket_closer']) === \false) {
                            break;
                        }
                    }
                }
                if ($afterVar !== \false && isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$afterVar]['code']])) {
