 ...
 * @since 8.2.0
 */
class ArgumentFunctionsUsageSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
         ...
         */
        if ($phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSURE)) === \false) {
            $isError = \false;
        ...
        if (isset($tokens[$closer]['parenthesis_owner']) && $tokens[$tokens[$closer]['parenthesis_owner']]['type'] === 'T_CLOSURE') {
            ...
            $prevNonEmpty = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $opener - 1, null, \true);
            if ($tokens[$prevNonEmpty]['code'] !== \T_STRING) {
                return;
            }
            $prevPrevNonEmpty = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $prevNonEmpty - 1, null, \true);
