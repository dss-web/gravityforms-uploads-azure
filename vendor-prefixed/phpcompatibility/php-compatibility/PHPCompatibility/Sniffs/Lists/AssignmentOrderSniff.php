 ...
 * @since 9.0.0
 */
class AssignmentOrderSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
         ...
         */
        $hasVars = $phpcsFile->findNext(array(\T_VARIABLE, \Dekode\GravityForms\Vendor\T_DOLLAR), $opener + 1, $closer);
        if ($hasVars === \false) {
            // Empty list, not our concern.
            return $closer + 1;
        }
        // Set the variable delimiters based on the list type being examined.
        $stopPoints = array(\Dekode\GravityForms\Vendor\T_COMMA);
        if ($tokens[$stackPtr]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY) {
            $stopPoints[] = \Dekode\GravityForms\Vendor\T_CLOSE_SHORT_ARRAY;
        } else {
            $stopPoints[] = \Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS;
        }
        $listVars = array();
        ...
        do {
            ...
            $nextStartPoint = $phpcsFile->findNext(array(\T_VARIABLE, \Dekode\GravityForms\Vendor\T_DOLLAR), $lastStopPoint + 1, $nextStopPoint);
