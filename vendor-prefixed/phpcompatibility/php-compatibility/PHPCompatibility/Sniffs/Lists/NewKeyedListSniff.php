 ...
 * @since 9.0.0
 */
class NewKeyedListSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
     ...
     * @var array
     */
    protected $sniffTargets = array(\T_LIST => \T_LIST, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY => \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY);
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY && $this->isShortList($phpcsFile, $stackPtr) === \false) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_LIST) {
            ...
            if ($nextNonEmpty === \false || $tokens[$nextNonEmpty]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS || isset($tokens[$nextNonEmpty]['parenthesis_closer']) === \false) {
     ...
     * @return void
     */
    protected function examineList(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $opener, $closer)
    {
        $start = $opener;
     ...
     */
    protected function hasTargetInList(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $start, $closer)
    {
        $tokens = $phpcsFile->getTokens();
        for ($i = $start + 1; $i < $closer; $i++) {
            ...
            if ($tokens[$i]['code'] === \T_LIST) {
                ...
                if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS && isset($tokens[$nextNonEmpty]['parenthesis_closer']) === \true) {
                    $i = $tokens[$nextNonEmpty]['parenthesis_closer'];
                }
            } elseif ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY && isset($tokens[$i]['bracket_closer'])) {
