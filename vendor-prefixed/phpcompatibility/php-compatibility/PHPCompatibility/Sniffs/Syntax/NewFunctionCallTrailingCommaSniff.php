 ...
 */
class NewFunctionCallTrailingCommaSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$nextNonEmpty]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS || isset($tokens[$nextNonEmpty]['parenthesis_closer']) === \false) {
            return;
        }
        ...
        $lastInParenthesis = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $closer - 1, $nextNonEmpty, \true);
        if ($tokens[$lastInParenthesis]['code'] !== \Dekode\GravityForms\Vendor\T_COMMA) {
            return;
        }
        $data = array();
