 ...
 * @since 9.0.0
 */
class NewFlexibleHeredocNowdocSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    {
        $targets = array(\T_END_HEREDOC, \Dekode\GravityForms\Vendor\T_END_NOWDOC);
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
         ...
         */
        if ($this->supportsBelow('7.2') === \true && \version_compare(\Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getVersion(), '2.6.0', '>=')) {
     ...
     * @return void
     */
    protected function detectIndentedNonStandAloneClosingMarker(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        if (\version_compare(\PHP_VERSION_ID, '70299', '>') === \true) {
             ...
             */
            $nextNonWhitespace = $phpcsFile->findNext(array(\T_WHITESPACE, \Dekode\GravityForms\Vendor\T_SEMICOLON), $stackPtr + 1, null, \true);
     ...
     * @return void
     */
    protected function detectClosingMarkerInBody(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        if (\version_compare(\PHP_VERSION_ID, '70299', '>') === \true) {
            ...
            if ($nextNonWhitespace === \false || $tokens[$nextNonWhitespace]['code'] === \Dekode\GravityForms\Vendor\T_SEMICOLON || ($tokens[$nextNonWhitespace]['code'] === \Dekode\GravityForms\Vendor\T_COMMA || $tokens[$nextNonWhitespace]['code'] === \Dekode\GravityForms\Vendor\T_STRING_CONCAT) && $tokens[$nextNonWhitespace]['line'] !== $tokens[$stackPtr]['line']) {
                // This is most likely a correctly identified closing marker.
                return;
            }
            // The real closing tag has to be before the next heredoc/nowdoc.
            $nextHereNowDoc = $phpcsFile->findNext(array(\T_START_HEREDOC, \Dekode\GravityForms\Vendor\T_START_NOWDOC), $stackPtr + 1);
            ...
            if (isset($tokens[$stackPtr]['scope_closer'], $tokens[$stackPtr]['scope_opener']) === \true && $tokens[$stackPtr]['scope_closer'] === $stackPtr) {
                ...
                $opener = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\T_START_NOWDOC, $stackPtr - 1);
                if ($opener === \false) {
                    return;
                }
            }
