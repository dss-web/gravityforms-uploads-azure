 ...
 * @since 9.2.0
 */
class NewArrayUnpackingSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $opener; $i < $closer;) {
            $i = $phpcsFile->findNext(array(\T_ELLIPSIS, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, \T_ARRAY), $i + 1, $closer);
            if ($i === \false) {
                return;
            }
            if ($tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY) {
