 ...
 */
class NewFunctionArrayDereferencingSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
     ...
     */
    public function isFunctionArrayDereferencing(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Next non-empty token should be the open parenthesis.
        $openParenthesis = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if ($openParenthesis === \false || $tokens[$openParenthesis]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS) {
            return array();
        }
        ...
        $prevToken = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if ($prevToken !== \false && \in_array($tokens[$prevToken]['code'], array(\T_DOUBLE_COLON, \T_OBJECT_OPERATOR), \true) === \false) {
            if ($tokens[$prevToken]['code'] === \Dekode\GravityForms\Vendor\T_BITWISE_AND) {
                // This may be a function declared by reference.
                $prevToken = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $prevToken - 1, null, \true);
            }
        ...
        $braces = array();
        do {
            $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $current + 1, null, \true, null, \true);
