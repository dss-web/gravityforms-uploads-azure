 ...
 */
namespace Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax;

use Dekode\GravityForms\Vendor\PHPCompatibility\Sniff;
use Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax\NewArrayStringDereferencingSniff;
use Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax\NewClassMemberAccessSniff;
use Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax\NewFunctionArrayDereferencingSniff;
 ...
 * @since 9.3.0
 */
class RemovedCurlyBraceArrayAccessSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    {
        $this->newArrayStringDereferencing = new \Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax\NewArrayStringDereferencingSniff();
        $this->newClassMemberAccess = new \Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax\NewClassMemberAccessSniff();
        $this->newFunctionArrayDereferencing = new \Dekode\GravityForms\Vendor\PHPCompatibility\Sniffs\Syntax\NewFunctionArrayDereferencingSniff();
    }
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($braces as $open => $close) {
            ...
            if ($tokens[$open]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET) {
                continue;
            }
            // Make sure there is something between the braces, otherwise it's still not curly brace array access.
            $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $open + 1, $close, \true);
     ...
     */
    protected function isVariableArrayAccess(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        $current = $stackPtr;
        $braces = array();
        do {
            $current = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $current + 1, null, \true);
            if ($current === \false) {
                break;
            }
            // Skip over square bracket array access. Bracket styles can be mixed.
            if ($tokens[$current]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET && isset($tokens[$current]['bracket_closer']) === \true && $current === $tokens[$current]['bracket_opener']) {
                $current = $tokens[$current]['bracket_closer'];
                continue;
            }
            // Handle property access.
            if ($tokens[$current]['code'] === \T_OBJECT_OPERATOR) {
                $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $current + 1, null, \true);
            ...
            if ($tokens[$current]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET) {
     ...
     *               brace array access was detected.
     */
    protected function isConstantArrayAccess(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        if ($tokens[$nextNonEmpty]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET || isset($tokens[$nextNonEmpty]['bracket_closer']) === \false) {
