 ...
 */
class ForbiddenBreakContinueVariableArgumentsSniff extends \Dekode\GravityForms\Vendor\PHPCompatibility\Sniff
{
    /**
    ...
    public function process(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $nextSemicolonToken = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_SEMICOLON, \T_CLOSE_TAG), $stackPtr, null, \false);
        $errorType = '';
        for ($curToken = $stackPtr + 1; $curToken < $nextSemicolonToken; $curToken++) {
            if ($tokens[$curToken]['type'] === 'T_STRING') {
                ...
                $openBracket = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $curToken + 1, null, \true);
