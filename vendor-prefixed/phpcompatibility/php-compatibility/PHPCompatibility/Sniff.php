 ...
 */
namespace Dekode\GravityForms\Vendor\PHPCompatibility;

use Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer_Exception as PHPCS_Exception;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer_File as File;
use Dekode\GravityForms\Vendor\PHP_CodeSniffer_Sniff as PHPCS_Sniff;
 ...
 * @since 5.6
 */
abstract class Sniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Sniff
{
    /**
    ...
    private function getTestVersion()
    {
        ...
        $testVersion = \trim(\Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getConfigData('testVersion'));
     ...
     * @return void
     */
    public function addMessage(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $message, $stackPtr, $isError, $code = 'Found', $data = array())
    {
        if ($isError === \true) {
     ...
     * @return bool
     */
    public function doesFunctionCallHaveParameters(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (\in_array($tokens[$stackPtr]['code'], array(\T_STRING, \T_ARRAY, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, \T_VARIABLE), \true) === \false) {
            return \false;
        }
        ...
        $nextNextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $nextNonEmpty + 1, $closeParenthesis + 1, \true);
     ...
     * @return int
     */
    public function getFunctionCallParameterCount(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
     ...
     * @return array
     */
    public function getFunctionCallParameters(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        if ($this->doesFunctionCallHaveParameters($phpcsFile, $stackPtr) === \false) {
            return array();
        }
        ...
        if ($tokens[$stackPtr]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY) {
            ...
            $opener = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        ...
        $paramStart = $opener + 1;
        $cnt = 1;
        while (($nextComma = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_COMMA, $tokens[$closer]['code'], \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, \Dekode\GravityForms\Vendor\T_CLOSURE), $nextComma + 1, $closer + 1)) !== \false) {
             ...
             */
            $hasNextParam = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $nextComma + 1, $closer, \true, null, \true);
     ...
     * @return array|false
     */
    public function getFunctionCallParameter(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $paramOffset)
    {
     ...
     */
    public function tokenHasScope(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $validScopes = null)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
     ...
     */
    public function inClassScope(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $strict = \true)
    {
        $validScopes = array(\T_CLASS);
        if (\defined('Dekode\\GravityForms\\Vendor\\T_ANON_CLASS') === \true) {
            $validScopes[] = \Dekode\GravityForms\Vendor\T_ANON_CLASS;
        }
        if ($strict === \false) {
     ...
     * @return string
     */
    public function getFQClassNameFromNewToken(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
        ...
        $start = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if ($start === \false) {
            return '';
        }
     ...
     * @return string
     */
    public function getFQExtendedClassName(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
        ...
        $extends = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::findExtendedClassName($phpcsFile, $stackPtr);
     ...
     * @return string
     */
    public function getFQClassNameFromDoubleColonToken(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
        ...
        if (\in_array($tokens[$stackPtr - 1]['code'], array(\Dekode\GravityForms\Vendor\T_PARENT, \T_STATIC), \true)) {
            return '';
        }
        // Get the classname from the class declaration if self is used.
        if ($tokens[$stackPtr - 1]['code'] === \Dekode\GravityForms\Vendor\T_SELF) {
     ...
     * @return string
     */
    public function getFQName(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $name)
    {
        if (\strpos($name, '\\') === 0) {
    ...
    {
        if (\strpos($FQName, '\\') !== 0) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Exception('$FQName must be a fully qualified name');
        }
     ...
     */
    public function determineNamespace(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
     ...
     */
    public function getDeclaredNamespaceName(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        ...
        $nextToken = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if ($tokens[$nextToken]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET) {
            /*
     ...
     */
    public function getReturnTypeHintToken(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        if (\defined('Dekode\\GravityForms\\Vendor\\T_RETURN_TYPE') && $tokens[$stackPtr]['code'] === \Dekode\GravityForms\Vendor\T_RETURN_TYPE) {
            return $stackPtr;
        }
        if ($tokens[$stackPtr]['code'] !== \T_FUNCTION && $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_CLOSURE) {
            return \false;
        }
        ...
        if (isset($tokens[$stackPtr]['scope_opener'])) {
            ...
            $nextSemiColon = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\T_SEMICOLON, $tokens[$stackPtr]['parenthesis_closer'] + 1, null, \false, null, \true);
        ...
        $hasColon = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_COLON, \Dekode\GravityForms\Vendor\T_INLINE_ELSE), $tokens[$stackPtr]['parenthesis_closer'] + 1, $endOfFunctionDeclaration);
        if ($hasColon === \false) {
            return \false;
        }
        /*
        ...
        $unrecognizedTypes = array(
            \T_CALLABLE,
            \Dekode\GravityForms\Vendor\T_SELF,
            \Dekode\GravityForms\Vendor\T_PARENT,
            \T_ARRAY,
            // PHPCS < 2.4.0.
            \T_STRING,
        );
     ...
     */
    public function getReturnTypeHintName(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // In older PHPCS versions, the nullable indicator will turn a return type colon into a T_INLINE_ELSE.
        $colon = $phpcsFile->findPrevious(array(\Dekode\GravityForms\Vendor\T_COLON, \Dekode\GravityForms\Vendor\T_INLINE_ELSE, \T_FUNCTION, \Dekode\GravityForms\Vendor\T_CLOSE_PARENTHESIS), $stackPtr - 1);
        if ($colon === \false || $tokens[$colon]['code'] !== \Dekode\GravityForms\Vendor\T_COLON && $tokens[$colon]['code'] !== \Dekode\GravityForms\Vendor\T_INLINE_ELSE) {
        ...
        for ($i = $colon + 1; $i <= $stackPtr; $i++) {
            ...
            if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$i]['code']])) {
                continue;
            }
            if ($tokens[$i]['type'] === 'T_NULLABLE') {
                continue;
            }
            if (\defined('Dekode\\GravityForms\\Vendor\\T_NULLABLE') === \false && $tokens[$i]['code'] === \Dekode\GravityForms\Vendor\T_INLINE_THEN) {
                // Old PHPCS.
                continue;
            }
     ...
     * @return bool
     */
    public function isClassProperty(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
     ...
     * @return bool
     */
    public function isClassConstant(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
     ...
     */
    protected function validDirectScope(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $validScopes)
    {
        $tokens = $phpcsFile->getTokens();
     ...
     */
    public function getTypeHintsFromFunctionDeclaration(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        if ($tokens[$stackPtr]['code'] !== \T_FUNCTION && $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_CLOSURE) {
            return array();
        }
        $parameters = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getMethodParameters($phpcsFile, $stackPtr);
     ...
     */
    public function getHashAlgorithmParameter(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
     ...
     * @return bool
     */
    public function isUseOfGlobalConstant(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($isLowPHPCS, $isLowPHP) === \false) {
            ...
            if (\version_compare(\PHP_VERSION_ID, '50400', '<')) {
                $isLowPHP = \true;
                $isLowPHPCS = \version_compare(\Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getVersion(), '2.4.0', '<');
            }
        }
        $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if ($next !== \false && ($tokens[$next]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS || $tokens[$next]['code'] === \T_DOUBLE_COLON)) {
        ...
        $prev = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        ...
        $firstOnLine = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $i + 1, null, \true);
        if ($firstOnLine !== \false && $tokens[$firstOnLine]['code'] === \T_USE) {
            $nextOnLine = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $firstOnLine + 1, null, \true);
            if ($nextOnLine !== \false) {
     ...
     */
    public function isPositiveNumber(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $start, $end, $allowFloats = \false)
    {
     ...
     *              (variable or calculations and such).
     */
    public function isNegativeNumber(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $start, $end, $allowFloats = \false)
    {
        $number = $this->isNumber($phpcsFile, $start, $end, $allowFloats);
        if ($number === \false) {
            return \false;
     ...
     *                        (variable or calculations and such).
     */
    protected function isNumber(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $start, $end, $allowFloats = \false)
    {
        $stringTokens = \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$heredocTokens + \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$stringTokens;
        $validTokens = array();
        $validTokens[\T_LNUMBER] = \true;
        $validTokens[\Dekode\GravityForms\Vendor\T_TRUE] = \true;
        // Evaluates to int 1.
        $validTokens[\Dekode\GravityForms\Vendor\T_FALSE] = \true;
        // Evaluates to int 0.
        $validTokens[\Dekode\GravityForms\Vendor\T_NULL] = \true;
        // Evaluates to int 0.
        if ($allowFloats === \true) {
        ...
        $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $start, $searchEnd, \true);
        while ($nextNonEmpty !== \false && ($tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_PLUS || $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_MINUS)) {
            if ($tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_MINUS) {
                $negativeNumber = $negativeNumber === \false ? \true : \false;
            }
            $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $nextNonEmpty + 1, $searchEnd, \true);
        }
        ...
        } elseif ($tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_TRUE) {
            $content = 1.0;
        } elseif ($tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_FALSE || $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_NULL) {
            $content = 0.0;
        } elseif (isset($stringTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
            if ($tokens[$nextNonEmpty]['code'] === \T_START_HEREDOC || $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_START_NOWDOC) {
                // Skip past heredoc/nowdoc opener to the first content.
                $firstDocToken = $phpcsFile->findNext(array(\Dekode\GravityForms\Vendor\T_HEREDOC, \Dekode\GravityForms\Vendor\T_NOWDOC), $nextNonEmpty + 1, $searchEnd);
                ...
                $nextNonEmpty = $phpcsFile->findNext(array(\T_END_HEREDOC, \Dekode\GravityForms\Vendor\T_END_NOWDOC), $nextNonEmpty + 1, $searchEnd);
     ...
     * @return bool
     */
    protected function isNumericCalculation(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $start, $end)
    {
        $arithmeticTokens = \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$arithmeticTokens;
        ...
        // phpcs:enable
        $skipTokens = \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens;
        $skipTokens[] = \Dekode\GravityForms\Vendor\T_MINUS;
        $skipTokens[] = \Dekode\GravityForms\Vendor\T_PLUS;
        ...
        while ($this->isNumber($phpcsFile, $subsetStart, $subsetEnd, \true) !== \false && isset($tokens[$arithmeticOperator + 1]) === \true) {
            ...
            if (\defined('T_POW') === \false && $tokens[$arithmeticOperator]['code'] === \Dekode\GravityForms\Vendor\T_MULTIPLY && $tokens[$arithmeticOperator + 1]['code'] === \Dekode\GravityForms\Vendor\T_MULTIPLY && isset($tokens[$arithmeticOperator + 2]) === \true) {
     ...
     */
    public function isShortTernary(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_INLINE_THEN) {
            return \false;
        }
        ...
        if ($tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_INLINE_ELSE) {
            return \true;
        }
        return \false;
    }
     ...
     * @return bool
     */
    public function isShortList(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false) {
        ...
        // Is this one of the tokens this function handles ?
        if ($tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY && $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_CLOSE_SHORT_ARRAY) {
            return \false;
        }
        switch ($tokens[$stackPtr]['code']) {
            case \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY:
                if (isset($tokens[$stackPtr]['bracket_closer']) === \true) {
                    $opener = $stackPtr;
                    $closer = $tokens[$stackPtr]['bracket_closer'];
                }
                break;
            case \Dekode\GravityForms\Vendor\T_CLOSE_SHORT_ARRAY:
         ...
         */
        $prevNonEmpty = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $opener - 1, null, \true, null, \true);
        if ($prevNonEmpty !== \false && $tokens[$prevNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_CLOSE_CURLY_BRACKET && isset($tokens[$prevNonEmpty]['bracket_opener']) === \true) {
            $maybeVariableVariable = $phpcsFile->findPrevious(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $tokens[$prevNonEmpty]['bracket_opener'] - 1, null, \true, null, \true);
            if ($tokens[$maybeVariableVariable]['code'] === \T_VARIABLE || $tokens[$maybeVariableVariable]['code'] === \Dekode\GravityForms\Vendor\T_DOLLAR) {
                return \false;
            }
        }
        $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $closer + 1, null, \true, null, \true);
        if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_EQUAL) {
            return \true;
        }
        ...
        $parentOpener = $opener;
        do {
            $parentOpener = $phpcsFile->findPrevious(array(\Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY, \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET), $parentOpener - 1, null, \false, null, \true);
        ...
        if (isset($tokens[$parentOpener]['bracket_closer']) === \true && $tokens[$parentOpener]['bracket_closer'] > $closer) {
            ...
            $phpcsVersion = \Dekode\GravityForms\Vendor\PHPCompatibility\PHPCSHelper::getVersion();
            if (\version_compare($phpcsVersion, '2.0', '>') === \true && \version_compare($phpcsVersion, '2.8', '<') === \true && $tokens[$parentOpener]['code'] === \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET) {
                $nextNonEmpty = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $tokens[$parentOpener]['bracket_closer'] + 1, null, \true, null, \true);
                if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Dekode\GravityForms\Vendor\T_EQUAL) {
                    return \true;
                }
                return \false;
     ...
     * @return bool
     */
    public function isVariable(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $start, $end, $targetNestingLevel)
    {
        static $tokenBlackList, $bracketTokens;
        // Create the token arrays only once.
        if (isset($tokenBlackList, $bracketTokens) === \false) {
            $tokenBlackList = array(\Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS => \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS, \Dekode\GravityForms\Vendor\T_STRING_CONCAT => \Dekode\GravityForms\Vendor\T_STRING_CONCAT);
            $tokenBlackList += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$assignmentTokens;
            $tokenBlackList += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$equalityTokens;
            $tokenBlackList += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$comparisonTokens;
            $tokenBlackList += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$operators;
            $tokenBlackList += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$booleanOperators;
            $tokenBlackList += \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$castTokens;
            /*
             * List of brackets which can be part of a variable variable.
             *
             * Key is the open bracket token, value the close bracket token.
             */
            $bracketTokens = array(\Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET => \Dekode\GravityForms\Vendor\T_CLOSE_CURLY_BRACKET, \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET => \Dekode\GravityForms\Vendor\T_CLOSE_SQUARE_BRACKET);
        }
        $tokens = $phpcsFile->getTokens();
     ...
     */
    public static function isUnaryPlusMinus(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_PLUS && $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_MINUS) {
            return \false;
        }
        $next = $phpcsFile->findNext(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if ($next === \false) {
        ...
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$operators[$tokens[$next]['code']]) === \true) {
        ...
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$operators[$tokens[$prev]['code']]) === \true) {
            // Just trying to operate on a positive/negative value; eg. ($var * -1).
            return \true;
        }
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$comparisonTokens[$tokens[$prev]['code']]) === \true) {
            // Just trying to compare a positive/negative value; eg. ($var === -1).
            return \true;
        }
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$booleanOperators[$tokens[$prev]['code']]) === \true) {
            // Just trying to compare a positive/negative value; eg. ($var || -1 === $b).
            return \true;
        }
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$prev]['code']]) === \true) {
            // Just trying to assign a positive/negative value; eg. ($var = -1).
            return \true;
        }
        if (isset(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_Tokens::$castTokens[$tokens[$prev]['code']]) === \true) {
            // Just casting a positive/negative value; eg. (string) -$var.
            return \true;
        }
        // Other indicators that a plus/minus sign is a unary operator.
        $invalidTokens = array(\Dekode\GravityForms\Vendor\T_COMMA => \true, \Dekode\GravityForms\Vendor\T_OPEN_PARENTHESIS => \true, \Dekode\GravityForms\Vendor\T_OPEN_SQUARE_BRACKET => \true, \Dekode\GravityForms\Vendor\T_OPEN_SHORT_ARRAY => \true, \Dekode\GravityForms\Vendor\T_COLON => \true, \Dekode\GravityForms\Vendor\T_INLINE_THEN => \true, \Dekode\GravityForms\Vendor\T_INLINE_ELSE => \true, \T_CASE => \true, \Dekode\GravityForms\Vendor\T_OPEN_CURLY_BRACKET => \true, \Dekode\GravityForms\Vendor\T_STRING_CONCAT => \true);
     ...
     */
    public function getCompleteTextString(\Dekode\GravityForms\Vendor\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $stripQuotes = \true)
    {
        $tokens = $phpcsFile->getTokens();
        // Must be the start of a text string token.
        if ($tokens[$stackPtr]['code'] !== \T_START_HEREDOC && $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_START_NOWDOC && $tokens[$stackPtr]['code'] !== \T_CONSTANT_ENCAPSED_STRING && $tokens[$stackPtr]['code'] !== \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING) {
            throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Exception('$stackPtr must be of type T_START_HEREDOC, T_START_NOWDOC, T_CONSTANT_ENCAPSED_STRING or T_DOUBLE_QUOTED_STRING');
        }
        if ($tokens[$stackPtr]['code'] === \T_CONSTANT_ENCAPSED_STRING || $tokens[$stackPtr]['code'] === \Dekode\GravityForms\Vendor\T_DOUBLE_QUOTED_STRING) {
            $prev = $phpcsFile->findPrevious(\T_WHITESPACE, $stackPtr - 1, null, \true);
            if ($tokens[$stackPtr]['code'] === $tokens[$prev]['code']) {
                throw new \Dekode\GravityForms\Vendor\PHP_CodeSniffer_Exception('$stackPtr must be the start of the text string');
            }
        }
        switch ($tokens[$stackPtr]['code']) {
            case \T_START_HEREDOC:
                $stripQuotes = \false;
                $targetType = \Dekode\GravityForms\Vendor\T_HEREDOC;
                $current = $stackPtr + 1;
                break;
            case \Dekode\GravityForms\Vendor\T_START_NOWDOC:
                $stripQuotes = \false;
                $targetType = \Dekode\GravityForms\Vendor\T_NOWDOC;
                $current = $stackPtr + 1;
                break;
            default:
