...
class TypeHintSniff implements \Dekode\GravityForms\Vendor\PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    private function checkForMissingArgumentHints(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, \Dekode\GravityForms\Vendor\NeutronStandard\SniffHelpers $helper)
    {
        $tokens = $phpcsFile->getTokens();
        $openParenPtr = $tokens[$stackPtr]['parenthesis_opener'];
        $closeParenPtr = $tokens[$stackPtr]['parenthesis_closer'];
        $hintTypes = [\T_STRING, \T_CALLABLE, T_SELF];
        // Support for phpcs < 3.3; see https://github.com/Automattic/phpcs-neutron-standard/issues/62
        if (\defined('Dekode\\GravityForms\\Vendor\\T_ARRAY_HINT')) {
        ...
        for ($ptr = $openParenPtr + 1; $ptr < $closeParenPtr; $ptr++) {
            if ($tokens[$ptr]['code'] === \T_VARIABLE) {
                ...
                if (!$tokenBeforePtr || !\in_array($tokenBefore['code'], $hintTypes, \true)) {
    ...
    private function checkForMissingReturnHints(\Dekode\GravityForms\Vendor\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, \Dekode\GravityForms\Vendor\NeutronStandard\SniffHelpers $helper)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        for ($ptr = $startOfFunctionPtr; $ptr < $endOfFunctionPtr; $ptr++) {
            ...
            if (empty($scopeClosers) && \in_array($token['code'], [\T_RETURN, \T_YIELD], \true)) {
        ...
        if ($hasNonVoidReturnType && ($nonVoidReturnCount === 0 || $voidReturnCount > 0)) {
            $errorMessage = $voidReturnCount > 0 ? 'Return type with void return' : 'Return type with no return';
            $errorType = $voidReturnCount > 0 ? 'IncorrectVoidReturn' : 'UnusedReturnType';
